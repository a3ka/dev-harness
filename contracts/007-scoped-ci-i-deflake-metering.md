...
**Срез 2 — новая ветвь `check_metering.sh` (изоляция портов, ДЕТЕРМИНИСТИЧНАЯ):**
- (порт-0) `gen_config` с `port: 0` → прокси биндит OS-эфемерный порт и РЕПОРТИТ фактический
  (stdout/файл); `check_metering` читает его, healthz на РЕПОРТНОМ порту зелёный. Красное против
  текущего (порт из cfg берётся как есть; healthz на `0` не отвечает → die). Флейк-фикс не
  тестируем флейк-тестом: атомарный `:0` устраняет free_port-скан+TOCTOU у КОРНЯ, изоляция
  под конкурентностью — СЛЕДСТВИЕ (каждый bind независим), не отдельный вероятностный прогон.
...
- `npm run check:metering` (расширенный ветвью порт-0) → 0.
...
- Различение в `verify_antiplacebo`: «0 задетых при резолвимом base» (→ exit 0, 0 прогонов) vs «нерезолвимый base» (→ полный прогон) — `scope_select` обязан вернуть РАЗНЫЕ сигналы; деталь реализации среза 1.
- Протокол репорта порта прокси при bind `:0` (stdout/файл) и как `check_metering` его читает — деталь реализации среза 2.
- Точный список код-барьеров яруса B (сужаемых) и граница «читает данные» по таблице `agent://GateScout` вопрос 3 — сверяется на круге критика.
