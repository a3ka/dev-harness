...
Д12 — зелёный контроль семьи `check_contract_ready` (спек-гейт 036 В4-3): 048 переиспользует её fixtures-каталог для калибровки (Н-155), барьер этим контрактом не правится, живая проба обязательна:

- `bash scripts/check_check_contract_ready.sh` → зелёный (rc=0), «check_check_contract_ready: ветви «all» зелены»
