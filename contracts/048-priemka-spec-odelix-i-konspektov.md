...
Д12 — зелёный контроль семьи `check_contract_ready` (спек-гейт 036 В4-2/В4-3): 048 переиспользует её fixtures-каталог для калибровки (Н-155), барьер этим контрактом не правится; проба живёт В САМОЙ семье и живьём вызывает барьер (argv-совместимость):

- `bash fixtures/check_check_contract_ready/doc_048/probe_zhivoy_vyzov_048.sh` → зелёный (rc=0), «check_check_contract_ready: ветви «all» зелены»
