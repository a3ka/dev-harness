...
Внутри представлены ID утверждения, его значение, статус as-is/to-be и ID
использованного источника (для to-be — решения), не только хеш пакета.
Непустые утверждения дают непустой фрагмент; изменение значения, статуса или
использованного источника отражается в его содержимом. Эти данные различимы как
самостоятельные значения, не подстроки чужого ID; полный формат строк свободен.
...
ничего не исправляет. Независимая проба читает содержимое между границами и
сопоставляет с сохранёнными в памяти фактами; --check не служит её оракулом.
...
и не легализует результат. Сохранность исходного проекта — отдельная обязанность:
срез до изменяющего probe сравнивается сразу после check, до восстановления входов,
как при принятии исходного значения, так и при отказе изменённому значению.
Изменение только изолированной копии само по себе не основание отказа.
...
| As-is/to-be различны; непустой formal-фрагмент независимо от --check предъявляет факты и меняется вслед за значением, статусом и использованным источником | `bash fixtures/check_check_contract_ready/red_doc_status_render.sh` |
| Изменяющий probe не меняет оракул; исходный проект сохранён непосредственно после принятия и отказа, до восстановления входов | `bash fixtures/check_check_contract_ready/red_doc_oracle.sh` |
...
положительный контроль rc проходит, но факты не генерируются, отрицательный
конформный doc-вход незаконно принимается. Это отсутствие поведения, не rc=127.
