#!/usr/bin/env node
/**
 * НЕ БАРЬЕР: CLI-реализация производных формальных фрагментов (контракт 027 §Док-пакет).
 * Генерация/сверка делается `scripts/render_document.sh` (барьер-обёртка), фикстура
 * держится за ним; эта программа вызывается через `exec` обёртки насквозь и собственных
 * rc 0/1/2 не объявляет как барьер. verify_antiplacebo читает эту строку как объявление
 ...
 *
 * возвраты (semantic, не якорь шапки): 0 — ок, 1 — именованное нарушение, 2 — нечем проверить.
 */
