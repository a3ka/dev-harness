...
if [ ! -d "$MAPDIR" ]; then
  # Исключение (контракт §Инварианты 5 п1): контракт-регистратор, на frozen-теге
  # которого consumers.d ещё нет (живой случай — ЭТОТ контракт 038).
  if git -C "$ROOT" tag -l 'frozen/contracts/*' | grep -q .; then
    die "потребители 116: маппинг не читается: $MAPDIR отсутствует"
  fi
  exit 0
fi
