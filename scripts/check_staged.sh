...
# (нет python3); 2 — нечем проверить (нет git, нет контрактов).
set -uo pipefail
...
# Находка 2 адверсария: `if … | python3 …` трактовал rc=127 от ненайденного python3 как
# «нет control-символа» — staged с переносом ВНУТРИ зоны проходил зелёным. Проверяем
# доступность инструмента ДО цикла и отказываем с названной причиной, когда грамматика
# имени не может быть исполнена. staged пуст либо автор не объявлен — оба пути вышли
# выше rc=0 до этой точки; здесь python3 запрашивается только когда дальше действительно
# нужна грамматическая проверка.
