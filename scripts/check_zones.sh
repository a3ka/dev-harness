...
# ГЕРМЕТИЧНОЕ ОКРУЖЕНИЕ — RE-EXEC ЧЕРЕЗ `env -i` (v6). ДО ЛЮБОЙ ДРУГОЙ СТРОКИ КОДА, даже до
...
#
# АБСОЛЮТНЫЕ ПУТИ У `env` И `bash` — ОБЯЗАТЕЛЬНЫ, А НЕ СТИЛЬ. Слово БЕЗ `/` в позиции команды
# ищется через таблицу ФУНКЦИЙ процесса раньше $PATH — ровно та дыра, которую этот блок
# закрывает; голый `exec env -i …` бесполезен, если среди отравленных функций есть и `exec`,
# и `env` (оба шэдуются идентично `unset`/`git` — живой прогон при подготовке фикса). Путь
# СО СЛЭШЕМ таблицу функций/builtin'ов/$PATH не консультирует вовсе — `execve()` напрямую,
# для любого числа и имён отравленных функций.
#
# УПРАВЛЕНИЕ НЕ ОПИРАЕТСЯ НА `exec`/`exit` КАК НА СЛОВА. Наивная форма `exec env -i … bash …`
# заменяет образ процесса, ТОЛЬКО если сам голый `exec` не отравлен; отравленный `exec`
# (шэдов-функция, например no-op) сделал бы re-exec молчаливым не-действием, и файл читался
# бы дальше в том же отравленном процессе. Симметрично, `…; exit $?` опирается на голый
# `exit`, шэдуемый тем же приёмом. Здесь ни один bare-word builtin для критического перехода
# не используется: `if`/`then`/`else`/`fi` — грамматика парсера (не simple-command, функцией
# не перекрывается никогда), сам re-exec — ОДНА команда с абсолютным путём, а код возврата
# наружу передаётся естественным поведением bash на EOF без `exit`: весь остаток файла (кроме
# самой этой строки re-exec) лежит в ветке `else`, поэтому в отравленном процессе исполняется
# РОВНО ОДНА команда, и по `fi` в конце файла процесс достигает EOF со статусом этой команды —
# тем же, что вернул дочерний чистый bash. Проверено живым прогоном ОДНОВРЕМЕННОГО шэдова
# `unset`+`git`+`exec`+`exit`+`env`: код возврата доходит байт-в-байт, ни одна из пяти теневых
# функций не вызывается ни разу.
#
...
if ( return 0 2>/dev/null ) || [[ "$(/usr/bin/tr '\0' '\n' < "/proc/$$/environ" 2>/dev/null)" != $'PATH=/usr/bin:/bin\nLC_ALL=C.UTF-8' ]]; then
  /usr/bin/env -i PATH=/usr/bin:/bin LC_ALL=C.UTF-8 /usr/bin/bash "${BASH_SOURCE[0]}" "$@"
else
set -euo pipefail
