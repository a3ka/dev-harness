...
while IFS= read -r nnn; do
  ...
  if [ "$declared" -eq 0 ]; then
    bad "$file: контракт не объявил ни зон, ни отказа из раздачи — нужна строка «ЗОНА <автор>: <путь>» либо «РАБОТА НЕ РАЗДАЁТСЯ: <причина>». Молчание не годится: «зон нет» и «зоны забыли» выглядят одинаково"
  fi
