...
while IFS= read -r f; do
  ...
  # Прецедент fixtures/.probe-only (контракт 034 инв.1/2, frontier п.1): каталог
  # с непустым маркером — опт-аут area-scan, файлы в нём считаются классом «п»
  # (не-барьер) без per-file шапки «Коды возврата:/НЕ БАРЬЕР:». Пустой или
  # отсутствующий маркер НЕ легализует (причина пуста — объявление без причины
  # неотличимо от опечатки). Применяется к scripts/ при конфликте грамматик:
  # замороженная грамматика files (038, «ровно 1 строка, никакого комментария»)
  # взаимноисключающая с шапкой-классификатором verify_antiplacebo, поэтому
  # носитель маркера выводится из области per-file сканирования целиком.
  parent="$(dirname -- "$f")"
  if [ "$parent" != "$SCRIPTS" ] && [ -s "$parent/.probe-only" ]; then
    continue
  fi
  h="$(header_of "$f")"
  case "$(classify "$h")" in
