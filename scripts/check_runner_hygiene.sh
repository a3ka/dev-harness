  ...
  sostav_pack() {  # <метка> <барьеров> <фикстур-на-барьер>
    ...
    while IFS= read -r ml; do
      ...
      if [ "$mk" = b ]; then
        ELOG="$EV/ev-$mu.log"
        if [ ! -s "$ELOG" ]; then
          die sostav "свидетельство вызовов барьера check_$mu не наблюдалось на непредсказуемом составе ($ELOG пуст) — раннер держит lock и run-каталог, но НЕ вызвал барьер НИ РАЗУ: гигиенические следы ≠ работа (находка 2 адверсария, круг 2)"
        fi
