...
#             И ВО ВРЕМЯ прогона (зонд 0.1с ловит «поработал в дереве — подмёл
#             trap'ом до выхода»), и после — обходы кругов 1–2 критика (Н-48-2);
...
#             §Предмет А.1 контракта 011 — ВНУТРИ раздела «Воркфлоу майлстоуна», и маркеры
...
#             contracts/010-*.md (аннотация, акт (i) implementer), — ДРУГОЙ и БОЛЕЕ
#             РАННИЙ коммит, чем добавивший verdicts/critic/contracts-010-v2.md с
#             первой строкой accept (акт (ii) critic). «Всё одним коммитом» и «вердикт
#             раньше аннотации» — обходы круга 2 критика.
#
...
# (green-root → red-root). КРАСНОЕ ПРОТИВ ТЕКУЩЕГО ДЕРЕВА: lock/race/scratch/scratchdef/
# chistka/norma/a010/porjadok красны (предмет не реализован), pgid/carveout зелены как охрана.
#
...
#     <ветвь>  — одна из: lock race scratch scratchdef chistka pgid norma carveout a010
#                porjadok (умолчание — все).
#
...
KNOWN="lock race scratch scratchdef chistka pgid norma carveout a010 porjadok"
if [ "$WANT" != all ]; then
...
if want scratchdef; then
  ...
  [ -z "$violation" ] || die scratchdef "во время default-прогона в стерегомом дереве появились новые пути (${violation% }) — при пустой VERIFY_ANTIPLACEBO_SCRATCH раннер обязан работать вне дерева; подметание trap'ом или rm до выхода дефект не закрывает: дерево грязно ВО ВРЕМЯ прогона, и параллельный снапшот его видит (Н-48-2, обход круга 2 критика)"
  ...
  ok '(scratchdef) пустая переменная → скратч с lock под ${TMPDIR:-/tmp}; дерево чисто и во время, и после'
fi
...
frozen-барьеров; полный прогон — только CI, от судьи он не требуется.'
  ...
  # РАЗДЕЛ (обход круга 2): дословный блок обязан жить ВНУТРИ раздела «Воркфлоу
  # майлстоуна» — текст в другом месте (архив отклонённых, цитата, code fence)
  # нормы в уставный раздел не вносит, а раздел может нести противоположное правило.
  # Границы раздела — от его заголовка до следующего «## »; «###» внутрь входят.
  sect="$(printf '%s\n' "$body" | awk '/^## Воркфлоу майлстоуна[[:space:]]*$/ { f = 1; next } f && /^## / { f = 0 } f')"
  has "$norm" "$sect" \
    || die norma "дословный блок нормы лежит ВНЕ раздела «Воркфлоу майлстоуна» — наличие текста в другом месте (архив отклонённых предложений, цитата, code fence с пометкой «не является нормой») не вносит норму в уставный раздел, а сам раздел может нести противоположное правило (обход круга 2 критика, Н-48-1)"
...
if want porjadok; then
  ...
  ok '(porjadok) аннотация 010 и вердикт 010-v2 — разные коммиты в верном порядке; вердикт accept'
fi
