...
#             существовавшее до прогона неприкосновенно — маркер кладётся и в
#             родительский $TMPDIR, и ПРЯМО в сам детерминированный default-скратч
#             (круг 1 критика 012: владение по факту пустой переменной rm -rf'ало
#             чужой существовавший каталог). Замер владельца: 47→49
...
# Круг 1 критика (012): маркер только в родительском $TMPDIR не доказывал
# неприкосновенности САМОГО default-скратча — раннер, безусловно ставящий
# признак владения по факту пустой переменной, rm -rf'ал существовавший каталог
# на EXIT, а ветвь видела лишь «новых путей нет» (прямой зонд критика:
# preexisting-файл исчезал при rc=0). Маркер кладётся ПРЯМО в детерминированный
# default-скратч этого дерева и обязан пережить прогон.
if want techka; then
  ...
  DEF="$SD/verify_antiplacebo-$(hash8_of "$T")"   # детерминированный default-скратч
  mkdir -p "$DEF"; printf 'preexisting\n' > "$DEF/preexisting"
  ...
  [ -e "$DEF/preexisting" ] || die techka "зачистка удалила существовавший до прогона default-скратч ($DEF) — убирает только создавший его прогон: признак владения по факту пустой переменной, без проверки существования каталога, удаляет чужое при rc=0 (райдер (ii) контракта 012, круг 1 критика). Хвост: $(tail200 "$WORK/tech-r1.out")"
