...
#
# Гоняет ПРЕДМЕТ `scripts/judge_gate.sh <sha>` и УТВЕРЖДАЕТ, что судья гейтит через
# CI-сигнал (Н-41). Детерминизм без сети: фикстура подставляет ФЕЙКОВЫЙ `check_ci_gate.sh`
# рядом с предметом, реагирующий на ЗНАЧЕНИЕ $1.
#
...
#   стаб-без-вызова-fake:   валит ОБЕ — нет маркера FAKE (не звал check_ci_gate);
#   стаб-игнорирующий-SHA:  валит (зелёный) — fake без matching SHA вернёт rc≠0.
#
# КРАСНОЕ ПРОТИВ ТЕКУЩЕГО ДЕРЕВА: judge_gate.sh ещё нет → run_gate → RC=2 маркер,
# обе ветви краснеют честно.
#
...
if want зелёный; then
  ...
  printf '  ok   (зелёный) зелёный CI → judge_gate RC=0, передал sha=%s\n' "$PASS_SHA" >&2
fi
