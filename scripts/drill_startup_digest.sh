...
case "$scenario" in
  real)
    ...
    # ── ЕДИНЫЙ СТРОГИЙ ПРИМИТИВ СВЕРКИ (контракт 015 круг 5, слово владельца:
    # структурная гарантия). Все проверки a–f идут ТОЛЬКО через этот примитив.
    #
    # assert_in_section <секция> <токен> <режим>
    #   секция — содержимое секции, извлечённое extract_section (структурный якорь
    #            между заголовками — выше).
    #   токен  — литеральная строка (для fxq) либо regex-фрагмент (для anchored).
    #            В anchored токен встраивается в regex напрямую, метасимволы
    #            `.[*+?|(){}` интерпретируются. Токены (b)/(c)/(f) управляемые
    #            (из EXPECT_* и переменных секции 2), новые вызовы должны это
    #            учитывать.
    #   режим:
    #     fxq       — токен обязан стоять самостоятельной строкой, побайтово равной
    #                 токену (под капотом: `grep -Fxq`). Покрывает (a), (d)-NOT, (e).
    #     anchored  — токен обязан стоять самостоятельной строкой; допускаются лишь
    #                 опц. ведущие/хвостовые пробелы и опц. непустой префикс вида
    #                 «<non-ws>+<ws>+» (префикс статус-short «?? »/«M »/«A » из
    #                 `git status --short` — формат контрактной печати
    #                 nabludenia_digest.sh).
    #                 Под капотом regex:
    #                   ^[[:space:]]*([^[:space:]]+[[:space:]]+)?<ТОКЕН>[[:space:]]*$
    #                 Токен обязан быть на КОНЦЕ строки: всё, что после него — только
    #                 пробелы. Это закрывает канал находки круга 5 (DIRTY, приклеенный
    #                 к диагностической строке вида «диагностика-прежней-ошибки:...:конец» —
    #                 после имени идёт не-пробельный текст, regex не совпадает).
    #                 Покрывает (b), (c), (f).
    # Возвращает 0, если токен найден самостоятельной строкой в секции; 1 — иначе.
    #
    # ─── ЗАПРЕТ ПОДСТРОЧНЫХ СВЕРОК (слово владельца: будущая 7-я проверка не должна
    # втащить подстроку молча) ───
    # Любая проверка токена на принадлежность секции ОБЯЗАНА идти через
    # assert_in_section. Использовать `grep -F -- <подстрока>` (или эквивалент) по
    # $out, по $sec или иному полному выводу ЗАПРЕЩЕНО — это прежний канал круга 4:
    # substring ловит токен в чужой строке («диагностика: ...; TAG», «архив ошибки:
    # ...; DIRTY») и принимает ложную раскладку стаба. ВСЕ проверки a–f ниже идут
    # через примитив; никаких параллельных substring-grep.
    assert_in_section() {
      local sec="$1" tok="$2" mode="$3"
      case "$mode" in
        fxq)
          printf '%s\n' "$sec" | LC_ALL=C grep -Fxq -- "$tok"
          ;;
        anchored)
          printf '%s\n' "$sec" | LC_ALL=C grep -Eq "^[[:space:]]*([^[:space:]]+[[:space:]]+)?${tok}[[:space:]]*\$"
          ;;
        *)
          printf 'INTERNAL: assert_in_section: неизвестный режим %s\n' "$mode" >&2
          return 2
          ;;
      esac
    }
    ...
    # Канал находки 4-го круга (substring-поиск по $out) закрыт fxq-режимом примитива:
    # токен обязан быть самостоятельной строкой, побайтово равной ожиданию.
    if ! assert_in_section "$OPEN_SEC" 'открытых: 0' fxq; then
    ...
    # Канал находки 4-го круга закрыт anchored-режимом примитива: токен обязан быть
    # самостоятельной строкой секции «дерево:» (substring «непушенных тегов: 1» в чужой
    # строке «диагностика: ...; TAG» отвергнут). Значение N извлекается из той же строки,
    # что прошла anchored-сверку (regex совпадает побайтово).
    if ! assert_in_section "$TREE_SEC" 'непушенных тегов: [0-9]+' anchored; then
      printf '  FAIL real: теги — токен «непушенных тегов: N» вне своей секции (нет самостоятельной строки в «дерево:»)\n' >&2
      exit 1
    fi
    tree_count_line="$(printf '%s\n' "$TREE_SEC" | LC_ALL=C grep -E '^[[:space:]]*([^[:space:]]+[[:space:]]+)?непушенных тегов: [0-9]+[[:space:]]*$' || true)"
    ...
    # (substring-поиск по $out ловил имя в «диагностика: ...; TAG») закрыт anchored.
    if ! assert_in_section "$TREE_SEC" "$TAG_NAME" anchored; then
    ...
    # Контракт требует структурного положения (НЕ-строкой), поэтому anchored-инверсия —
    # не подстрочная сверка: токен обязан быть самостоятельной строкой, иначе сюда не
    # попадает (substring «статус: чисто» в чужой строке нас бы не смутил — нужна ровно
    # строка, и anchored-инверсия это требует).
    if assert_in_section "$TREE_SEC" 'статус: чисто' fxq; then
    ...
    if ! assert_in_section "$TREE_SEC" 'статус: аномалии' fxq; then
      printf '  FAIL real: статус — токен «статус: аномалии» вне своей секции (нет самостоятельной строки в «дерево:»)\n' >&2
      exit 1
    fi

    # (f) DIRTY — самостоятельной строкой аномалии в секции «дерево:» в формате
    # `git status --short` (digest печатает «  <status-short> DIRTY», для untracked — «??»).
    # Канал находки круга 5 (substring-поиск DIRTY по $out или по TREE_SEC ловил имя в
    # «архив ошибки: ...; DIRTY» или в «диагностика-прежней-ошибки:<DIRTY>:конец» —
    # ложная раскладка стаба проходила) закрыт anchored-режимом примитива: regex требует,
    # чтобы после токена до конца строки шли ТОЛЬКО пробелы (regex:
    # ^[[:space:]]*([^[:space:]]+[[:space:]]+)?TOK[[:space:]]*$), поэтому диагностические
    # суффиксы «:конец», «; TAG» и пр. отвергнуты. НИКАКОГО отдельного substring-grep DIRTY
    # здесь нет и быть не должно — иначе (f) снова становится слабее остальных.
    if ! assert_in_section "$TREE_SEC" "$DIRTY" anchored; then
      printf '  FAIL real: статус — имя untracked «%s» вне своей секции (нет самостоятельной строки аномалии в «дерево:»)\n' "$DIRTY" >&2
      exit 1
    fi
