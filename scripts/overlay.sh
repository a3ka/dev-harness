...
for prov in $KEYED; do
  ...
  grep -qE "^\s+${prov}:" "$HERE/.omp/models.yml" \
    || die "провайдер $prov назначен, но в .omp/models.yml не объявлен — маршрут учёта отсутствует"
  grep -A2 -E "^\s+${prov}:" "$HERE/.omp/models.yml" | grep -q 'METERING_PROXY_URL' \
    || die "провайдер $prov идёт не через прокси учёта: потолок расхода не выполняет своей работы"
