 ...
 *
 * НЕ БАРЬЕР: это реализация прокси учёта, а не барьер приёмки. Барьер — `scripts/check_metering.sh`,
 * его фикстуры лежат в `fixtures/check_metering/`. Этот файл — предмет проверки, а не
 * её исполнитель; возвращаемые коды (0/1/2) — собственные коды прокси, а не классификация
 * verify_antiplacebo. Заголовок «КОДЫ ВОЗВРАТА» в шапке носит описательный характер, и
 * verify_antiplacebo без явного «НЕ БАРЬЕР:» счёл бы файл неприсвоенным.
 */
