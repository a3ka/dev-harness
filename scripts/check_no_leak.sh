...
#               архитектора 024).
# ─── САНИТИЗАЦИЯ ОКРУЖЕНИЯ (структурный фикс круга k5 — default-deny whitelist) ──
# Контрпример блокер 1 (S-cnl-sanitized-guard-off): часовой `_CNL_SANITIZED` сам
# был флагом в АТАКУЮЩЕМ окружении — единственным присваиванием `_CNL_SANITIZED=1`
# пропускался `exec env -i`, и весь входящий env (BASH_ENV/GIT_DIR/GIT_WORK_TREE)
# доживал до тела детектора нетронутым. Закрытие: часовой — НЕ «булевый флаг»,
# а ПОДМНОЖЕСТВО текущего окружения явному whitelist'у. default-deny: всё, чего
# нет в whitelist'е, удаляется через `env -i KEY=VAL ...`. Идемпотентность: после
# первой же re-exec с whitelist_only — все переменные «в whitelist'е», проверка
# сразу истинна, рекурсии нет.
#
# Контрпримеры блокеров 2/3 (S-home-git-config-excludes / S-home-fsmonitor-tracked):
# HOME пропускался из входящего окружения и `git status` подтягивал
# `$HOME/.gitconfig` с core.excludesFile / core.fsmonitor, скрывавший утечку И
# модификацию tracked-файла. Закрытие: HOME принудительно задаётся внутри
# whitelist'а в /nonexistent (утилиты, которым нужен HOME для записи, упадут
# fail-closed явно), И дополнительно GIT_CONFIG_GLOBAL/SYSTEM=/dev/null блокирует
# чтение git'ом И `$HOME/.gitconfig`, И `/etc/gitconfig`. Тот же механизм лечит и
# деривативы (trustLevel в attacker-controlled .gitconfig) — git не читает ничего
# из home/cwd, кроме собственного .git/config (легитимный; см. Демаркацию).
#
# Контрмеры пространства имён: `env`/`bash`/итоговая util-обвязка резолвятся
# АБСОЛЮТНЫМ путём из /usr/bin /bin /usr/local/bin — тот же класс «мимо PATH»
# мог бы подменить и их. Остаточный когнитивный предел: если сам BASH_ENV успел
# переопределить `exec`/`printf`/`[` ДО этой строки — вне демонстрированного
# адверсарием класса (омнипотентный атакующий внутри того же процесса —
# вне самозащиты скрипта).
_cnl_WHITELIST='_CNL_SANITIZED _CNL_WHITELIST PATH HOME TMPDIR LC_ALL LANG LC_COLLATE LC_CTYPE LC_MESSAGES LC_MONETARY LC_NUMERIC LC_PAPER LC_NAME LC_TELEPHONE LC_TIME GIT_CONFIG_GLOBAL GIT_CONFIG_SYSTEM GIT_CONFIG_NOSYSTEM PWD SHLVL _'
# Идемпотентный цикл: пока в окружении ЕСТЬ что-то не из whitelist'а — re-exec
# с ТОЛЬКО whitelist'ом (HOME=/nonexistent, TMPDIR=/tmp, GIT_CONFIG_*=/dev/null).
# Бесконечной рекурсии нет: после первого re-exec все переменные «в whitelist'е»,
# цикл сразу выходит и идём дальше.
_cnl_untrusted=1
while [ "$_cnl_untrusted" = 1 ]; do
  _cnl_untrusted=0
  while IFS='=' read -r _cnl_k _cnl_v; do
    [ -n "$_cnl_k" ] || continue
    case " $_cnl_WHITELIST " in
      *" $_cnl_k "*) ;;
      *) _cnl_untrusted=1; break ;;
    esac
  done < <(env -0)
_cnl_TMPDIR="${TMPDIR:-/tmp}"
  if [ "$_cnl_untrusted" = 1 ]; then
    _cnl_env=""
    ...
    exec "$_cnl_env" -i \
      _CNL_SANITIZED=1 \
      _CNL_WHITELIST="$_cnl_WHITELIST" \
      _CNL_TMPDIR="$_cnl_TMPDIR" \
      PATH="$_cnl_trusted" \
      HOME=/nonexistent \
      TMPDIR="$_cnl_TMPDIR" \
      LC_ALL=C \
...
unset _cnl_untrusted _cnl_WHITELIST _cnl_TMPDIR _cnl_env _cnl_bash _cnl_trusted _cnl_p _cnl_d
...
# два вызова с разными cwd сходятся в один файл (И-7). TMPDIR после subset-re-exec
# жёстко задан в /tmp (наблюдение G: TMPDIR из АТАКУЮЩЕГО окружения нельзя
# пропускать — снап попадал под snap-каталог атакующего, и `[L]` на $SNAP/$SNAP_DIR
# не ловит). hash8 — через bash-substring (${X:0:8}), не внешний cut: cut тоже
# пришлось бы пинить, substring — builtin, не зависит от PATH.
...
unset _canonsum
# TMPDIR уважается (фикстуры используют свой TMPDIR для само-очистки); в проде
# TMPDIR=/tmp после re-exec. Компонент-симлинк-чек (наблюдения G/H) идёт ниже
# в do_check — каждое звено пути от ${TMPDIR:-/tmp} до $SNAP ОБЯЗАНО быть
# регулярным каталогом/файлом, НЕ симлинком.
...
# DOT-GIT МАНИФЕСТ (находка 5, S-dot-git-leak). Контрпример: `.git/hooks/*` и
# `.git/config` лежат ВНУТРИ основного чекаута и НЕ отражаются в porcelain
# (`git status --porcelain -uall` их не выводит по построению). Подмена активного
# `.git/hooks/pre-push` отключала сам pre-push-барьер (форма исходного инцидента
# 2026-09-10, `.githooks/pre-push`). Закрытие: ОТДЕЛЬНЫЙ manifest — sha256
# содержимого `.git/hooks/*` (рекурсивно по не-симлинк-узлам) и `.git/config` —
# шьётся в общий снапшот/сверку строками `DOTGIT:<sha>\t<path>`. Грамматика
# отличается от `XY:` (нет двухсимвольного префикса), сортируется вместе.
#
# emit_dotgit_manifest <канон-корень> — рекурсивный обход .git/hooks и одного
# файла .git/config. Внутренний rc=2 (sha256sum не смог прочесть) распространяется
# вверх так же, как у emit_manifest (`|| return 2`). Симлинки ПРОПУСКАЮТСЯ на
# каждом уровне — они могут dangle или вести в attacker-controlled каталог; их
# присутствие само по себе не отражается в манифесте, и подмена через симлинк
# не пройдёт (нет строки ⇒ чистка по Демаркации, но и утечки нет — симлинк
# атакующего не в нашем отпечатке; проверка пути на симлинк делается ДО этого
# обхода).
emit_dotgit_manifest_walk() {  # <dir> <prefix-от-канон-корня>
  local dir="$1" pre="$2" entry p fp
  for entry in "$dir"/*; do
    [ -e "$entry" ] || continue
    ...
    if [ -f "$entry" ]; then
      ...
      fp="${fp%% *}"
      # entry ведущий слэш съедаем: «/<dir>/<basename>» → «<dir-basename>…»
      printf 'DOTGIT:%s\t%s/%s\n' "$fp" "$pre" "${entry##*/}"
    elif [ -d "$entry" ]; then
...
emit_dotgit_manifest() {  # <канон-корень>
  local gitdir="$1/.git" hooksdir entry fp
  ...
  # 2. .git/config (один файл — точка контроля trustLevel/receive.denyCurrentBranch
    # и пр., которые влияют на всё поведение git; подмена эквивалентна
    # конфигурированию «зеркального» репозитория).
...
# Итоговая manifest() сшивает porcelain + dot-git и сортирует ЕДИНЫМ sort.
# Под «единой сортировкой» означает: hash8-снимок стабилен между двумя вызовами
# вне зависимости от того, что porcelain/dot-git вернули порознь (sort на общем
# стриме), и verify-sha обновляется в do_snapshot при том же содержимом.
#
# pipefail локально: «Левый side пайпа — сабшелл, переменные в нём не выживают
# наружу» (приём v4 неявный). С pipefail exit of pipeline = правый-most non-zero,
# и emit_manifest rc 2 (git status / sha256sum / rev-parse отказ) корректно
# прорастает до $? после `m=$(...)` в do_snapshot без подмены sort'ом.
manifest() {
  set -o pipefail
  {
    emit_manifest "$1" "${2:-}"
    emit_dotgit_manifest "$1"
  } | "$SORT"
}
do_snapshot() {
  local m manifest_rc
...
do_check() {
  ...
  # ЗАЩИТА-СНИМКА — полная проверка пути (контрпример наблюдения H, вердикт k5):
  # каждый КОМПОНЕНТ пути от TMPDIR-корня до $SNAP включительно обязан быть
  # реальным каталогом/файлом, НЕ симлинком. Защищает от:
  #   * прямой подмены $SNAP симлинком (мера 1 в.к2 адверсария 01ed097);
  #   * подмены $SNAP_DIR симлинком (мера 1б);
  #   * подмены ПРОМЕЖУТОЧНОГО компонента ${TMPDIR:-/tmp}/dev-harness-leak/<hash8>
  #     симлинком — ранее проходило мимо -L, атакующий перенаправлял снап в свой
  #     каталог и подменял файл снимка «свежим», проходящим сверку (наблюдение H);
  #   * подмены ПРОМЕЖУТОЧНОГО компонента $TMPDIR/dev-harness-leak тем же приёмом
  #     без симлинка-родителя (наблюдение G — TMPDIR пропускался).
  # Реализация: ровно один проход по разбиению пути на абсолютные компоненты.
  # Bash не имеет `$path_parts` builtin; используем `IFS='/' read -ra` (read — builtin,
  # не требует пина, и не подменяется PATH-атакой).
  local cnl_pfx="" cnl_i=1 cnl_n=0 cnl_part
  local -a cnl_parts=()
  IFS='/' read -r -a cnl_parts <<< "$SNAP"
  cnl_n="${#cnl_parts[@]}"
  # cnl_parts[0]="" от ведущего '/'; первый реальный компонент — cnl_parts[1].
  while [ "$cnl_i" -lt "$cnl_n" ]; do
    cnl_pfx="${cnl_pfx}/${cnl_parts[$cnl_i]}"
    if [ -L "$cnl_pfx" ]; then
      printf 'ОТКАЗ: компонент пути снимка — симлинк: %s (replacement промежуточного каталога недопустим; наблюдение H к5)\n' "$cnl_pfx" >&2
      exit 1
    fi
    cnl_i=$((cnl_i+1))
  done
  # Сам $SNAP (последний компонент) — отдельная явная проверка после цикла,
  # для ясного сообщения.
  if [ -L "$SNAP" ]; then
