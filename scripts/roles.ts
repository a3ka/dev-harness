...
const MODEL_ROLE: Record<string, string> = {
  ...
  implementer: 'task',    // дешёвая кодовая полоса; НЕ `default`, потому что `default` —
                          // модель живой сессии, а ей нужен vision (я смотрю кадры)
  adversary: 'advisor',   // ломает проверку исполнением; ДРУГОЕ семейство
  critic: 'advisor',      // судит план ДО заморозки; судит работу автора, значит обязан
                          // быть вне его семейства — то же основание, что у adversary
