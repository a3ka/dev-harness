...
# ── Скретч трассы В4-2/В4-3 (обход fe5e4f73, критик: «различающий критерий
# для живого вызова»): живой вызов барьера доказывается ИСПОЛНЕНИЕМ, не
# текстом пробы. `SHELLOPTS=xtrace` в окружении bash-процесса распространяется
# на КАЖДЫЙ вложенный bash (включая пробу-файл, запущенную по шебангу
# `#!/usr/bin/env bash`, на любую глубину) без ре-парсинга $back и без
# явного `bash -x` на каждом уровне: каждый такой процесс сам пишет свои
# `+`-строки исполненных команд в унаследованный stderr. Строка, стоящая
# ПОСЛЕ раннего exit/return внутри пробы, физически не появляется в трассе —
# это и есть различающий критерий, которого не давал грамматический разбор
# исходника (обход case_control.sh: `exit 0` + мёртвая строка с литералом
# барьера кредитовалась В4-2 текстом, хотя барьер не вызывался ни разу).
...
while IFS= read -r line; do
  ...
  # под SHELLOPTS=xtrace (см. комментарий у _V4_TRACE_DIR): грамматический
  # отказ барьера в выводе ПЕРВОГО прогона ($out/$rc, без -x) — несовместимость,
  # НЕЗАВИСИМО от заявленной автором причины (закрывает А2-дыру: диспетчер-фраза,
  # скопированная в суффикс, маскирует мёртвую пробу под «легитимно красную»;
  # оракул — живой вызов, не заявленный текст, правило 8). Литерал пути барьера
  # ищется В ТРАССЕ исполнения, не в исходнике пробы (закрывает обход fe5e4f73:
  # текстовый литерал после недостижимого `exit 0` credитовался старой grep-по-
  # файлу веткой — трасса такую строку физически не несёт, раз она не исполнена).
  if [ -n "${probe_path:-}" ]; then
    ...
    if [ -n "$_v4_fam" ] && [ -z "${_v4_family_in_edit[$_v4_fam]:-}" ] \
      ...
      if [ -n "$_v4_trace" ]; then
        ( cd "$ROOT" && timeout 60 env SHELLOPTS=xtrace bash -c "$back" ) \
          >/dev/null 2>"$_v4_trace" || true
      fi
      if [ -n "$_v4_trace" ] && [ -s "$_v4_trace" ] \
         && grep -Fq "scripts/check_${_v4_fam}.sh" "$_v4_trace"; then
