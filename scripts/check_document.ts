#!/usr/bin/env node
/**
 * НЕ БАРЬЕР: CLI-реализация приёмки doc-пакета (контракт 027 §Док-пакет). Приёмка
 * делается `scripts/check_document.sh` (барьер-обёртка), фикстура держится за ним;
 * эта программа вызывается через `exec` обёртки насквозь и собственных rc 0/1/2
 * не объявляет как барьер. verify_antiplacebo читает эту строку как объявление
 ...
 *
 * возвраты (semantic, не якорь шапки): 0 — машинная мера выполнена,
