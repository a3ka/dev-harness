...
#      обязан быть объявлен в `fixtures/verify_ci_parity/exceptions.txt` с записанной
#      причиной. Файл читает САМ барьер, поэтому выйти из правила молча нельзя.
#
...
# записана в `fixtures/verify_ci_parity/exceptions.txt` — молчаливого выбора быть не
...
EXCEPTIONS="$ROOT/fixtures/verify_ci_parity/exceptions.txt"
