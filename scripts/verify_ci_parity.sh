...
#
# Сейчас три таких ключа с разными причинами (образец именованного обоснования —
# `check_metering`: Н-103 + слово владельца + cognitive-only компенсация):
...
#     verify_antiplacebo/case_*.sh`) гоняются ТОЛЬКО полным прогоном `verify_antiplacebo
#     .sh`, которого в CI после чистого cutover больше нет (анти-плацебо-запусков: 1 —
#     шардный, прогон 1 ревьюера к1). КОМПЕНСАЦИЯ: scoped-прогоны гоняют раннер НА
#     КАЖДОМ шарде (`npm run check:antiplacebo -- --scope ${{ matrix.keys }}`, шаг
#     «Анти-плацебо · шард apN»), и раннер сам исполняет свои же 23 case фикстуры
#     через FIFO-протокол — сам-тест покрыт косвенно на каждом пуше, отдельный шаг
#     для этого не нужен. Без `keys:` в шарде `scope_select` отверг бы ключ и сломал
#     матрицу; full-прогон отсутствует по дизайну (Н-48: дорого и не масштабируется).
#     Каталог `fixtures/verify_antiplacebo/` (23 case) остаётся, покрытие обеспечено
#     исполнителем каждого шарда.
