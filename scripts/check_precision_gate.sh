...
#       прямая (не-трейсовая) самопроверка case-файла зелена;
...
if [ "${#FAMILIES[@]}" -gt 0 ]; then
  ...
  for key in "${!FAMILIES[@]}"; do
    ...
    for cf in "${case_files[@]}"; do
      ...
      [ "$direct_rc" -eq 0 ] || die "fixtures/check_$key/$cf: самопроверка провалена (rc $direct_rc, ожидался 0)"
      ...
      [ "$live" -eq 1 ] || die "fixtures/check_$key/$cf: барьер $key не вызван живьём (/proc-наблюдение не нашло exec)"
      [ "$traced_rc" -eq 0 ] || die "fixtures/check_$key/$cf: самопроверка провалена (rc $traced_rc, ожидался 0)"
    done
  done
fi
