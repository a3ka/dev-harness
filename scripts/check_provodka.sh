...
#   (verdicts/adversary/contracts-038-markdown-fence-terminator.md): контракт
...
for ((i=header_idx+1; i<${#all_lines[@]}; i++)); do
  ...
  case "$ln" in
    ...
    "ПРОВОДКА-ЭНФОРСМЕНТ:"*) continue ;;   # В1: вне поля, ищется по всему файлу (г0-доп)
    "- "*) channel_lines+=("$ln") ;;
    *[![:space:]]*) die "проводка: строка вне грамматики: $ln" ;;
  esac
done
