...
for ch in "${channels[@]}"; do
  ...
  case "$kind" in
    ...
    role)
      ...
      # Грамматика role= (замороженный контракт 038 §Инварианты 1): ТОЛЬКО
      # `roles/<роль>.md`, ровно один сегмент, никаких `policies/r.txt` или
      # иных путей.
      #
      # Вердикт адверсария b9f0c3d вскрыл glob-ловушку: в bash `case` шаблон
      # `roles/[^/]*.md` НЕ является POSIX-ERE — `[^/]` действует на РОВНО
      # ОДИН символ, а последующий `*` матчит ЛЮБЫЕ символы, включая `/`.
      # Поэтому `roles/../policies/r.md` проходил case (первый не-слэш после
      # `roles/` — `.`, дальше `*` поглощал `../policies/r`, суффикс `.md`
      # сходил), а проверка `[ -f "$ROOT/$path" ]` нормализовывала путь в
      # `policies/r.md` — обход гейта и done-тег.
      #
      # Фикс (вариант а из контракта): двухстадийный case. Стадия 1 —
      # грубый матч `roles/*.md` фиксирует суффикс `.md` и общую форму;
      # стадия 2 — после снятия префикса `roles/` явная проверка, что
      # остаток НЕ содержит `/`, что и есть НАСТОЯЩЕЕ ограничение
      # «ровно один сегмент, без слэшей внутри».
      case "$path" in
        roles/*.md)
          rest="${path#roles/}"
          case "$rest" in
