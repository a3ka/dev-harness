...
for ch in "${channels[@]}"; do
  ...
  case "$kind" in
    ...
    role)
      ...
      # г4 — норма-строка ИЩЕТСЯ В ЦЕЛИ: `grep` сам разыменовывает symlink,
      # поэтому резолв В плоский `roles/<роль>.md` (см. §Семантика 1) ищется
      # по таргету — отдельной «проверки нормы в резолве» не требуется.
