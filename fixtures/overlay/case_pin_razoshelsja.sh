...
#
# Пин на зелёном прогоне называет сумму подставного бинаря — так и должно быть: пин
# описывает ФАКТИЧЕСКИ установленный харнес, а установлен здесь подставной.
set -euo pipefail
...
PATH="$WORK/bin:$PATH" BARRIER_ROOT="$WORK" "$BARRIER" --check
