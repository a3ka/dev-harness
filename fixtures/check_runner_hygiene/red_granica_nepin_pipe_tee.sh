...
# пишет ТОЛЬКО в allowlist Г3 (dev-harness-verify ∪ artifact:// и внутренние
# URI); всякая иная цель — чекаут, чужой каталог, свободный абсолют — block
...
# (сегодняшняя) форма — пара fail-open ветвей:
#   • pathAllowed: «worktree === null || canonicalWt === null → return true»
#     — непиннованный bash: всякий абсолютный операнд разрешён;
#   • judgeEditWrite: «worktree === null → return pass» — непиннованный
#     edit/write: абсолют свободен.
# Честная форма: обе ветви при worktree:null пропускают ТОЛЬКО allowlist
# (isAllowedURI ∪ isWithin(dev-harness-verify)); иное — block Н-85.
#
# ВЕТВИ (7):
...
printf 'ЗЕЛЁНОЕ 025-дыра-B: fail-closed предъявлен — непиннованная запись block Н-85 на чекаут-цели (1) и свободном абсолюте (4), pass только на скратч (2) и artifact (3); относительный вектор блок (5); под пинном чекаут-цель блок (6), в-пинне pass (7)\n' >&2
exit 0
