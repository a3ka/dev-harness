#!/usr/bin/env bash
# НЕ БАРЬЕР: СЛАБАЯ ФОРМА детектора 024 — «константа '-' недостижимого HEAD»
# (правка-круг 3, вердикт адверсария d67ac4b; v4 same_class_preventive —
# форма эпохи до превентива, жива в коде ab69279:120).
# СЛАБОСТЬ: rev-parse HEAD в submodule-рекурсии при отказе молча даёт
# константу '-' («@head:-»): отказ producer'а выглядит успехом, истинная
# ревизия submodule потеряна.
# Дельта от честной формы (stab_detektor_chestnyj.sh): ТОЛЬКО ветвь
# rev-parse-отказа; предпроверка утилит, фиксация rc status, именованный
# отказ нечитаемого файла сохранены.
# УМИРАЕТ на воротах 18 red_detektor_utechek.sh (свой вход: gitlink с
# inner-репозиторием на orphan-ветке — rev-parse rc 128, status жив): rc 0 с
# «@head:-» вместо именованного rc 2 «HEAD недостижим». Причина смерти:
# «константа '-'».
set -uo pipefail
export LC_ALL=C
...
emit_manifest() {  # <канон-корень> <префикс-путей>
  ...
  while IFS= read -r -d '' entry; do
    ...
    if [ -d "$full" ] && [ -e "$full/.git" ]; then
      ...
      # СЛАБОСТЬ: недостижимый HEAD — молчаливая константа '-'.
      head="$(git -C "$full" rev-parse HEAD 2>/dev/null || printf -- '-')"
      printf '%s:@head:%s\t%s%s\n' "$xy" "$head" "$prefix" "$path"
      emit_manifest "$full" "$prefix$path/" || { rm -f -- "$tmpf"; return 2; }
    elif [ -e "$full" ]; then
      # обычный файл — sha256 байтов; нечитаемый — именованный отказ, НЕ константа.
