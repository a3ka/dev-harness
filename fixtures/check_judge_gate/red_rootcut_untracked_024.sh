...
#                снимка) + утечка;
#   контроль 4 — инвариант ПРЕДСТАВЛЕНИЯ с оракулом ЗНАЧЕНИЯ, ДВУХСОСТОЯННЫЙ
#                (арбитраж orakul-podsmotrennaya-konstanta-024, РЕШЕНИЕ вопрос
#                1): untracked-путь со СЛУЧАЙНЫМИ на каждый прогон именем и
#                байтами, живущий под ignore-правилом ЕЩЁ ДО снимка, представлен
#                в манифесте снимка строкой с sha256, СВЕРЕННЫМ с вычисленным
#                пробой ДО вызова субъекта — в состоянии A и, после перезаписи
#                байтов на B≠A, в состоянии B (форма без значения — именованный
#                ОТКАЗ «несовпадение отпечатка»; заморозка/эхо между A и B —
#                «представление не следит за байтами»; различение текущее/
#                целевое);
...
# TMPDIR редиректится в WORK, имена утечки случайны КАЖДЫЙ прогон,
# оракулируемый вход контроля-4 (имя И байты SEED, а также mk_main) — случаен
# на КАЖДЫЙ прогон отдельным источником энтропии (entropy(), см. ниже), toy
...
#               нога отсутствует / представление не следит за байтами /
#               утечка исчезла с диска).
set -uo pipefail
...
# Оракул независимой исходной истины (тот же корень, что блокер вердикта
# критика 873b81b в tracked-пробе): проба вычисляет ожидаемый отпечаток
# САМА — пин sha256sum из TRUSTED_PATH ровно как в субъекте
# (/usr/bin /bin /usr/local/bin -> command -v) + sanity-хэш пустого ввода
# (класс S-path-forged-sha256 субъекта).
TP=""
...
# Живой git-чекаут toy: tracked-объект — СЛУЧАЙНЫЕ имя и байты на каждый
# прогон (РЕШЕНИЕ арбитража orakul-podsmotrennaya-konstanta-024, точка правки
# :98; не оракулируется этой пробой, но словарный литерал запрещён тем же
# правилом — единообразие с tracked-пробой). Имя/байты — глобально в
# TNAME/BYTES_MAIN.
mk_main() {  # <каталог>
  mkdir -p "$1"
...
# ─── контроль 4: инвариант ПРЕДСТАВЛЕНИЯ, оракул ЗНАЧЕНИЯ, ДВЕ ФАЗЫ ────────────
# Целевой манифест (v6 нога-2) обязан представлять КАЖДЫЙ untracked-путь,
# несущий sha256 БАЙТОВ — В ТОМ ЧИСЛЕ путь, живущий под ignore-правилом ЕЩЁ
# ДО снимка (стабильный, немутирующий), И СЛЕДИТЬ за изменением байтов между
# двумя снимками (арбитраж orakul-podsmotrennaya-konstanta-024, РЕШЕНИЕ
# вопрос 1, п.2). Оракул — НЕЗАВИСИМАЯ исходная истина (блокер вердикта
# критика 873b81b) с энтропией входа (п.1 того же РЕШЕНИЯ): имя и байты SEED
# случайны на каждый прогон, словарный литерал запрещён (З1). Проба вычисляет
# отпечаток байтов ДО каждого вызова субъекта (правило 8: ожидания в памяти
# пробы) и требует СОВПАДЕНИЯ вычисленного значения в строке манифеста как
# САМОСТОЯТЕЛЬНОГО токена (границы — не-hex символы). ФОРМА БЕЗ ЗНАЧЕНИЯ
# ЗАПРЕЩЕНА: любой 64-hex, не равный вычисленному отпечатку, — именованный
# ОТКАЗ «несовпадение отпечатка» (фаза A). ЗАМОРОЗКА/ЭХО первого снимка
# (строка фазы B несёт тот же отпечаток, что и фаза A, при изменённых
# байтах, либо строка вовсе пропала) — именованный ОТКАЗ «представление не
# следит за байтами» (фаза B). Текущий код такие пути не представляет вовсе
# (замер E1: hid_seed отсутствует в снимке) → именованный ОТКАЗ «нога
# отсутствует».
...
SEED="hid_seed_${RANDOM}_$(entropy).txt"
BYTES_A4="SEED UNDER RULE $(entropy)"
printf '%s\n' "$BYTES_A4" > "$K4/$SEED"
# Ожидание ДО вызова субъекта (правило 8): отпечаток состояния A.
WANT_A4="$("$SHA256SUM_PIN" -- "$K4/$SEED")" \
  || { printf 'ОТКАЗ контроль-4 (оракул): sha256sum не смог прочитать %s\n' "$K4/$SEED" >&2; exit 1; }
WANT_A4="${WANT_A4%% *}"
...
# Строка манифеста — сравнением ПОСЛЕДНЕГО tab-поля с путём (чистый bash,
# без regex-экранирования случайного имени), затем сверка ЗНАЧЕНИЯ.
seed_line() {  # <файл-снимка> -> заполняет $SNAPLINE (строка с последним полем = SEED)
  local snap="$1" l4
  SNAPLINE=""
...
seed_line "$SNAP"
# ── фаза A: строка обязана нести sha(A) ─────────────────────────────────────────
if [ -z "$SNAPLINE" ]; then
...
printf '%s\n' "$SNAPLINE" | grep -qE "(^|[^0-9a-f])${WANT_A4}([^0-9a-f]|$)" \
  || fail контроль-4 "несовпадение отпечатка" \
    "024-v6 (фаза A): строка $SEED не несёт ВЫЧИСЛЕННОГО пробой отпечатка $WANT_A4 (форма без значения: 64-hex отсутствует либо не равен sha256 байтов файла; строка: $SNAPLINE) — корневой срез не реализован"
# ── фаза B: перезапись SEED на случайное B≠A → повторный снимок того же корня ──
BYTES_B4="SEED UNDER RULE $(entropy)"
[ "$BYTES_B4" != "$BYTES_A4" ] \
  || { printf 'ОТКАЗ контроль-4 (энтропия): состояние B совпало с A — свежая выборка обязана отличаться\n' >&2; exit 1; }
printf '%s\n' "$BYTES_B4" > "$K4/$SEED"
WANT_B4="$("$SHA256SUM_PIN" -- "$K4/$SEED")" \
  || { printf 'ОТКАЗ контроль-4 (оракул): sha256sum не смог прочитать %s\n' "$K4/$SEED" >&2; exit 1; }
WANT_B4="${WANT_B4%% *}"
[ "$WANT_B4" != "$WANT_A4" ] \
  || { printf 'ОТКАЗ контроль-4 (энтропия): sha(B)=sha(A) при разных байтах — коллизия окружения\n' >&2; exit 1; }
run_subj "$K4" --snapshot "$K4"
[ "$SUBJ_RC" -eq 0 ] || fail контроль-4 "повторный снимок" "повторный снимок отказал rc=$SUBJ_RC. Вывод: $SUBJ_OUT"
seed_line "$SNAP"
[ -n "$SNAPLINE" ] || fail контроль-4 "представление не следит за байтами" \
  "024-v6 (фаза B): после перезаписи $SEED на состояние B строка пути исчезла из манифеста — sha(B) не появился (замороженное представление первого снимка) — корневой срез не реализован"
printf '%s\n' "$SNAPLINE" | grep -qE "(^|[^0-9a-f])${WANT_B4}([^0-9a-f]|$)" \
  || fail контроль-4 "представление не следит за байтами" \
    "024-v6 (фаза B): строка $SEED не несёт ПЕРЕВЫЧИСЛЕННОГО отпечатка $WANT_B4 после перезаписи байтов (эхо/заморозка первого снимка; строка: $SNAPLINE) — корневой срез не реализован"
printf '%s\n' "$SNAPLINE" | grep -qE "(^|[^0-9a-f])${WANT_A4}([^0-9a-f]|$)" \
  && fail контроль-4 "представление не следит за байтами" \
    "024-v6 (фаза B): строка $SEED всё ещё несёт отпечаток состояния A $WANT_A4 (замороженное представление; обязан появиться sha(B)=$WANT_B4, исчезнуть sha(A); строка: $SNAPLINE) — корневой срез не реализован"
ok контроль-4 "инвариант представления, две фазы: untracked-путь под правилом несёт sha(A), после перезаписи — ПЕРЕВЫЧИСЛЕННЫЙ sha(B)≠sha(A)"
...
printf 'red_rootcut_untracked_024: 6 контролей зелены (v6 нога-2: НОВЫЙ untracked-файл при любом ignore-носителе даёт rc 1 С ИМЕНЕМ утечки; untracked-путь под правилом представлен байтами в манифесте и следит за их изменением между снимками — двухсостоянная сверка, оракулируемый вход случаен каждый прогон; Б6-контроль и env-санитизация не регрессируют)\n' >&2
exit 0
