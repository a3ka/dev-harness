...
#                снимка) + утечка;
#   контроль 4 — инвариант ПРЕДСТАВЛЕНИЯ с оракулом ЗНАЧЕНИЯ: untracked-путь,
#                живущий под ignore-правилом ЕЩЁ ДО снимка, представлен в
#                манифесте снимка строкой с sha256, СВЕРЕННЫМ с вычисленным
#                пробой ДО вызова субъекта (форма без значения — любой
#                чужой 64-hex — именованный ОТКАЗ; различение текущее/целевое);
...
# ─── контроль 4: инвариант ПРЕДСТАВЛЕНИЯ, оракул ЗНАЧЕНИЯ ──────────────────────
...
# ДО снимка (стабильный, немутрирующий). Оракул — НЕЗАВИСИМАЯ исходная
# истина (тот же корень, что блокер вердикта критика 873b81b): проба САМА
# вычисляет отпечаток байтов SEED ДО вызова субъекта (правило 8: ожидания в
# памяти пробы, снимок не источник ожиданий) и требует СОВПАДЕНИЯ
# вычисленного значения в строке манифеста как САМОСТОЯТЕЛЬНОГО токена
# (границы — не-hex символы). ФОРМА БЕЗ ЗНАЧЕНИЯ ЗАПРЕЩЕНА: любой 64-hex,
# не равный вычисленному отпечатку (зеркальная контрмодель DECORATION на
# untracked-ноге), — именованный ОТКАЗ «несовпадение отпечатка», не зачёт.
# Текущий код такие пути не представляет вовсе (замер E1: hid_seed
# отсутствует в снимке) → именованный ОТКАЗ «нога отсутствует».
...
# Ожидание ДО вызова субъекта: sha256 байтов SEED, вычисленный пробой.
WANT4="$("$SHA256SUM_PIN" -- "$K4/$SEED")" \
...
printf '%s\n' "$SNAPLINE" | grep -qE "(^|[^0-9a-f])${WANT4}([^0-9a-f]|$)" \
  || fail контроль-4 "несовпадение отпечатка" \
    "024-v6: строка $SEED не несёт ВЫЧИСЛЕННОГО пробой отпечатка $WANT4 (форма без значения: 64-hex отсутствует либо не равен sha256 байтов файла; строка: $SNAPLINE) — корневой срез не реализован"
ok контроль-4 "инвариант представления: untracked-путь под правилом несёт ВЫЧИСЛЕННЫЙ пробой sha256 байтов"
