...
#      + снимок не тронут. Стаб «тихая деградация пустого реестра» (rc 0 + пусто,
#      прочитанные как здоровое «зон нет — путь не вердиктный») умирает здесь:
#      ослеплённый реестр не доказывает зону;
...
#      снимок не тронут; стаб запасного пути (префикс/кэш при ненулевом zones_load)
#      умирает ЗДЕСЬ, не на р9: реализация, честно отказывающая на rc 0-пусто и
#      падающая в запасной путь при rc≠0, проходит р0–р9 (замер 2-B арбитра 9f86d1c).
#
