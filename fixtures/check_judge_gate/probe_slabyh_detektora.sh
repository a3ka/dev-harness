...
# 4d1d265, блокер 4) + правка-круг 3 (вердикт d67ac4b: фазы 15-18 —
# S-git-status-1, S-no-sha256sum, ERR-константа, константа '-'). Н-39: привязка стаба к входу — КОДОМ фикстур, не прозой
...
#   фаза 1  — честная форма (stab_detektor_chestnyj.sh) → red rc 0: все 18
#              ворот проходят против эталона договора целиком (правка-круг 3:
#              форма догнана до ab69279 + превентивы v4);
...
$out"
printf '  ok   честная форма: 18 ворот зелены\n' >&2
...
slobaja stab_detektor_glotaet_rc_statusa.sh        15 'S-git-status-1'              'глотает-rc-status: отказ git status выглядит успехом'
slobaja stab_detektor_bez_predproverki.sh          16 'S-no-sha256sum'              'без-предпроверки: rc 127 утилиты выглядит успехом'
slobaja stab_detektor_err_konstanta.sh             17 'ERR-константа'               'ERR-константа: нечитаемый путь молчит'
slobaja stab_detektor_tire_konstanta.sh            18 "константа '-'"               'константа-тире: недостижимый HEAD молчит'
...
printf 'пойманы по имени ворота и причине: всегда-чисто, снимок-в-дереве, без-снимка-пропускает, первый-снимок-навсегда, своя-cwd, обеих-cwd, тихий-cwd, porcelain-множество, свёртка-каталогов, без-gitlinkов, без-абс-корня, без-root-строки, запись-при-сверке, глотает-rc-status, без-предпроверки, ERR-константа, константа-минус; честная форма проходит все 18 ворот; плацебо exit-1 отвергнуто\n' >&2
exit 0
