...
# 4d1d265; правка-круг 3 — вердикт адверсария d67ac4b: +ворота 15-18) —
# детектор утечек основного чекаута scripts/check_no_leak.sh
...
#   15 (а, зелёно-охранная) отказ ИСТОЧНИКА манифеста (S-git-status-1, блокер 1
#      d67ac4b) → rc 2 именованный «манифест не прочитан» на снимке И сверке;
#      слабая форма «глотание rc» (process substitution) обязана умирать.
#      Правка-круг 3 (8911b68): инъекция отказа — нечитаемый .git/index ЖИВОГО
#      git (chmod 000: rev-parse --git-dir rc 0, status rc 128, замерено), НЕ
#      PATH-обёртка: после пин-фикса к3 фейковый git в PATH детектора НЕ
#      достигает (GIT из TRUSTED_PATH) — старая инъекция на фикс-коде давала бы
#      ворота fail-open (ждём rc 2, код честно работает rc 0); живой отказ
#      недостижим для пина, инвариант не ослаблен ни в одну сторону;
#   16 (б, зелёно-охранная) отсутствие утилиты (S-no-sha256sum, блокер 2
#      d67ac4b) → rc 2 именованный «утилита … отсутствует»; вход — уже-грязный
#      tracked-путь с дописью: ERR-константа делает допись невидимой;
#   17 (в, честно-красная) нечитаемый файл (v4 same_class_preventive: «ERR»-
#      константа) → rc 2 именованный «не смог прочитать <путь>», снимок И
#      сверка — фраза сверена с реализатором (Impl024prev) побайтово;
#   18 (г, честно-красная) недостижимый rev-parse HEAD в submodule-рекурсии
#      (v4: константа '-') → rc 2 именованный «HEAD недостижим в <путь>»,
#      снимок И сверка — фраза сверена с реализатором побайтово.
...
#
# ПРИРОДА ворот 15-18 на коде ab69279: (а)/(б) ЗЕЛЁНО-ОХРАННЫЕ — предпроверка
# утилит и фиксация rc status уже в реализации, ворота лишь охраняют от слабых
# форм; (в)/(г) ЧЕСТНО-КРАСНЫЕ — константы ERR/'-' в коде ещё живут, ворота
# красны до правки имплементера (параллельная пачка превентивов), после неё
# зеленеют вместе с честной формой стаба.
#
...
run_subj_path() {  # <каталог-PATH-префикс> <cwd> <аргументы...> — PATH-обман
  local bin="$1" cwd="$2"; shift 2
  SUBJ_OUT="$( cd "$cwd" && PATH="$bin:$PATH" bash "$SUBJ" "$@" 2>&1 )" \
    && SUBJ_RC=0 || SUBJ_RC=$?
}

run_subj_bin() {  # <каталог-единственный-PATH> <cwd> <аргументы...> — урезанный PATH
  local bin="$1" cwd="$2"; shift 2
  SUBJ_OUT="$( cd "$cwd" && PATH="$bin" bash "$SUBJ" "$@" 2>&1 )" \
    && SUBJ_RC=0 || SUBJ_RC=$?
}
...
# ── ворота 15: отказ источника манифеста → rc 2 именованный (S-git-status-1) ──
# ЗЕЛЁНО-ОХРАННАЯ (rc producer'а фиксируется переменной через mktemp-буфер —
# код ab69279; слабые формы «глотание rc» умирают). Инъекция — правка-круг 3
# (8911b68): нечитаемый .git/index живого git — status rc 128 при живом
# rev-parse --git-dir (замерено probe1 архитектора); PATH-обёртка более НЕ
# вход: пин-фикс к3 резолвит GIT из TRUSTED_PATH, фейк в PATH игнорируется, и
# ворота на фикс-коде молча перестали бы собираться (ожидался rc 2 — код честно
# вернул бы rc 0): fail-open ворот хуже слабой инъекции. Живой отказ источника
# пином НЕ обходится — инвариант «отказ producer'а ≠ молчаливый успех» судится
# на обоих кодах (до/после пина) одной мерой.
M15="$WORK/v15_status_otkaz_${RANDOM}"
mk_main "$M15"
printf 'uzhe_grjaznyj_do_snimka\n' >> "$M15/.githooks/pre-push"  # уже-грязный tracked (требование 2 d67ac4b)
chmod 000 "$M15/.git/index"       # status rc 128; rev-parse --git-dir rc 0 (не читает индекс)
run_subj "$M15" --snapshot "$M15"
[ "$SUBJ_RC" -eq 2 ] || fail 15 "отказ status" "S-git-status-1: --snapshot rc=$SUBJ_RC (ожидался 2) — отказ git status проглочен и выглядит успехом (блокер 1 d67ac4b: process substitution глотает rc producer'а). Вывод: $SUBJ_OUT"
has "манифест не прочитан" || fail 15 "отказ status" "S-git-status-1: --snapshot без именованной фразы «манифест не прочитан». Вывод: $SUBJ_OUT"
M15B="$WORK/v15b_svodka_${RANDOM}"             # вход сверки: отказ ПОСЛЕ честного снимка
mk_main "$M15B"
run_subj "$M15B" --snapshot "$M15B"             # честный снимок — нормальный индекс
[ "$SUBJ_RC" -eq 0 ] || fail 15 "отказ status" "S-git-status-1: честный снимок (читаемый индекс) отказал rc=$SUBJ_RC. Вывод: $SUBJ_OUT"
printf 'utechka_posle_snimka\n' >> "$M15B/.githooks/pre-push"
chmod 000 "$M15B/.git/index"     # источник манифеста умирает ПОСЛЕ снимка
run_subj "$M15B" --check "$M15B"
[ "$SUBJ_RC" -eq 2 ] || fail 15 "отказ status" "S-git-status-1: --check rc=$SUBJ_RC (ожидался 2) — отказ источника манифеста на сверке превращается в пустой манифест и «чисто». Вывод: $SUBJ_OUT"
has "манифест не прочитан" || fail 15 "отказ status" "S-git-status-1: --check без именованной фразы «манифест не прочитан». Вывод: $SUBJ_OUT"
if has "$P_CHISTO"; then
  fail 15 "отказ status" "S-git-status-1: сверка при отказавшем status объявила «$P_CHISTO» — отказ, выглядящий успехом. Вывод: $SUBJ_OUT"
fi
chmod 644 "$M15/.git/index" "$M15B/.git/index"  # гигиена toy (умрут trap'ом, но индекс возвращаем)
ok 15 "отказ источника манифеста (status rc≠0) → rc 2 именованный на снимке и сверке"

# ── ворота 16: отсутствие утилиты → rc 2 именованный (S-no-sha256sum) ─────────
# ЗЕЛЁНО-ОХРАННАЯ (код ab69279 уже закрыт предпроверкой закрытого списка).
# PATH содержит только git bash sort comm mkdir cat mktemp rm — НЕТ sha256sum
# и НЕТ cut (ровно окружение S-no-sha256sum, блокер 2 d67ac4b); вход —
# уже-грязный tracked-путь с дописью после снимка: ERR-константа делает
# допись невидимой (дельта « M:ERR» пуста).
M16="$WORK/v16_net_utilit_${RANDOM}"
mk_main "$M16"
printf 'uzhe_grjaznyj_do_snimka\n' >> "$M16/.githooks/pre-push"  # уже-грязный tracked (требование 2 d67ac4b)
BIN16="$WORK/v16-bin"
mkdir -p "$BIN16"
for u in git bash sort comm mkdir cat mktemp rm; do
  ln -s "$(command -v "$u")" "$BIN16/$u"
done
run_subj_bin "$BIN16" "$M16" --snapshot "$M16"
[ "$SUBJ_RC" -eq 2 ] || fail 16 "нет sha256sum" "S-no-sha256sum: --snapshot rc=$SUBJ_RC (ожидался 2) — rc 127 утилиты мимо PATH проглочен и выглядит успехом (блокер 2 d67ac4b). Вывод: $SUBJ_OUT"
has "утилита sha256sum отсутствует" || fail 16 "нет sha256sum" "S-no-sha256sum: отказ без имени утилиты. Вывод: $SUBJ_OUT"
printf 'utechka_posle_snimka\n' >> "$M16/.githooks/pre-push"     # допись в УЖЕ-грязный путь
run_subj_bin "$BIN16" "$M16" --check "$M16"
[ "$SUBJ_RC" -eq 2 ] || fail 16 "нет sha256sum" "S-no-sha256sum: --check rc=$SUBJ_RC (ожидался 2) — ERR-константа делает допись в уже-грязный путь невидимой: «чисто» при отказавшей утилите. Вывод: $SUBJ_OUT"
has "утилита sha256sum отсутствует" || fail 16 "нет sha256sum" "S-no-sha256sum: --check без имени утилиты. Вывод: $SUBJ_OUT"
if has "$P_CHISTO"; then
  fail 16 "нет sha256sum" "S-no-sha256sum: сверка при отсутствующей sha256sum объявила «$P_CHISTO». Вывод: $SUBJ_OUT"
fi
ok 16 "отсутствие утилиты → rc 2 именованный, допись не невидима"

# ── ворота 17: нечитаемый файл → rc 2 именованный, НЕ ERR-константа ───────────
# ЧЕСТНО-КРАСНАЯ: на коде ab69279 отпечаток нечитаемого — молчаливая константа
# ERR (два нечитаемых состояния неотличимы, допись в нечитаемый уже-грязный
# путь невидима); v4 same_class_preventive — именованный fail-closed
# «не смог прочитать <путь>». Фраза сверена с реализатором (Impl024prev)
# побайтово: rc 2 на снимке И сверке, ДО решения «чисто».
M17="$WORK/v17_nechitaemyj_${RANDOM}"
mk_main "$M17"
NECHIT17="nechitaemyj_${RANDOM}.txt"
: > "$M17/$NECHIT17"
chmod 200 "$M17/$NECHIT17"      # write-only: чтение запрещено, допись возможна
run_subj "$M17" --snapshot "$M17"
[ "$SUBJ_RC" -eq 2 ] || fail 17 "нечитаемый файл" "ERR-константа: --snapshot rc=$SUBJ_RC (ожидался 2) — нечитаемый путь молча сведён в константу ERR: два нечитаемых состояния неотличимы, допись в нечитаемый уже-грязный путь невидима (v4 same_class_preventive). Вывод: $SUBJ_OUT"
has "не смог прочитать" || fail 17 "нечитаемый файл" "ERR-константа: --snapshot без именованной фразы «не смог прочитать». Вывод: $SUBJ_OUT"
has "$NECHIT17" || fail 17 "нечитаемый файл" "ERR-константа: имя нечитаемого $NECHIT17 не названо. Вывод: $SUBJ_OUT"
rm -f "$M17/$NECHIT17"          # честный повторный снимок для входа сверки
run_subj "$M17" --snapshot "$M17"
[ "$SUBJ_RC" -eq 0 ] || fail 17 "нечитаемый файл" "честный повторный снимок отказал rc=$SUBJ_RC. Вывод: $SUBJ_OUT"
: > "$M17/$NECHIT17"
chmod 200 "$M17/$NECHIT17"      # нечитаемое появляется ПОСЛЕ снимка
run_subj "$M17" --check "$M17"
[ "$SUBJ_RC" -eq 2 ] || fail 17 "нечитаемый файл" "ERR-константа: --check rc=$SUBJ_RC (ожидался 2) — нечитаемый путь на сверке обязан отказывать именованно как окружение, а не решать по молча деградировавшему манифесту. Вывод: $SUBJ_OUT"
has "не смог прочитать" || fail 17 "нечитаемый файл" "ERR-константа: --check без именованной фразы «не смог прочитать». Вывод: $SUBJ_OUT"
has "$NECHIT17" || fail 17 "нечитаемый файл" "ERR-константа: --check не назвал $NECHIT17. Вывод: $SUBJ_OUT"
ok 17 "нечитаемый файл → rc 2 именованный на снимке и сверке"

# ── ворота 18: недостижимый HEAD submodule → rc 2, НЕ константа '-' ───────────
# ЧЕСТНО-КРАСНАЯ: на коде ab69279 rev-parse HEAD в submodule-рекурсии молча
# даёт константу '-' («@head:-»); v4 same_class_preventive — тот же
# fail-closed «HEAD недостижим в <путь>». Механика: inner-репозиторий на
# orphan-ветке (unborn HEAD) — rev-parse rc 128, status жив (проверено).
# Фраза сверена с реализатором (Impl024prev) побайтово.
M18="$WORK/v18_head_nedostizhim_${RANDOM}"
mk_main "$M18"
mk_gitlink "$M18" "submod"
env "${GTOY_ENV[@]}" git -C "$M18/submod" checkout -q --orphan golova-net
run_subj "$M18" --snapshot "$M18"
[ "$SUBJ_RC" -eq 2 ] || fail 18 "HEAD недостижим" "константа '-': --snapshot rc=$SUBJ_RC (ожидался 2) — rev-parse HEAD в submodule-рекурсии молча сведён в «@head:-»: отказ producer'а выглядит успехом (v4 same_class_preventive). Вывод: $SUBJ_OUT"
has "HEAD недостижим" || fail 18 "HEAD недостижим" "константа '-': --snapshot без именованной фразы «HEAD недостижим». Вывод: $SUBJ_OUT"
has "submod" || fail 18 "HEAD недостижим" "константа '-': путь submodule не назван. Вывод: $SUBJ_OUT"
M18B="$WORK/v18b_svodka_${RANDOM}"             # вход сверки: поломка ПОСЛЕ честного снимка
mk_main "$M18B"
mk_gitlink "$M18B" "submod"
run_subj "$M18B" --snapshot "$M18B"
[ "$SUBJ_RC" -eq 0 ] || fail 18 "HEAD недостижим" "честный снимок здорового gitlink отказал rc=$SUBJ_RC. Вывод: $SUBJ_OUT"
env "${GTOY_ENV[@]}" git -C "$M18B/submod" checkout -q --orphan golova-net
run_subj "$M18B" --check "$M18B"
[ "$SUBJ_RC" -eq 2 ] || fail 18 "HEAD недостижим" "константа '-': --check rc=$SUBJ_RC (ожидался 2) — сверка при недостижимом HEAD submodule обязана отказывать именованно, а не решать по молча деградировавшему манифесту. Вывод: $SUBJ_OUT"
has "HEAD недостижим" || fail 18 "HEAD недостижим" "константа '-': --check без именованной фразы «HEAD недостижим». Вывод: $SUBJ_OUT"
ok 18 "недостижимый HEAD submodule → rc 2 именованный на снимке и сверке"

printf 'red_detektor_utechek: 18 ворот зелены (14 круга 2 + 15-18 круга 3, вердикт d67ac4b)\n' >&2
exit 0
