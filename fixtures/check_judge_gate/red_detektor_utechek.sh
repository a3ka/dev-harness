...
#   15 (а, зелёно-охранная) отказ `git status` (S-git-status-1, блокер 1
...
# ── ворота 15: отказ git status → rc 2 именованный (S-git-status-1) ───────────
# ЗЕЛЁНО-ОХРАННАЯ (код ab69279 уже закрыт: rc producer'а фиксируется переменной
# через mktemp-буфер); слабые формы «глотание rc» умирают. Контрольный git в
# PATH: всё передаётся настоящему git, подкоманда status — отказ rc 1 — ровно
# отказ источника манифеста из S-git-status-1 (блокер 1 d67ac4b).
...
BIN15="$WORK/v15-bin"
mkdir -p "$BIN15"
cat > "$BIN15/git" <<'WRAPGIT'
#!/usr/bin/env bash
# контрольный git класса S-git-status-1 (вердикт d67ac4b): всё — настоящему
# git, подкоманда status — отказ rc 1 (отказ источника манифеста).
for a in "$@"; do
  [ "$a" = status ] && exit 1
done
exec /usr/bin/git "$@"
WRAPGIT
chmod +x "$BIN15/git"
run_subj_path "$BIN15" "$M15" --snapshot "$M15"
...
run_subj "$M15" --snapshot "$M15"              # настоящий снимок — нормальный PATH
[ "$SUBJ_RC" -eq 0 ] || fail 15 "отказ status" "S-git-status-1: настоящий снимок (нормальный PATH) отказал rc=$SUBJ_RC. Вывод: $SUBJ_OUT"
run_subj_path "$BIN15" "$M15" --check "$M15"
...
ok 15 "отказ git status → rc 2 именованный на снимке и сверке"
