...
# ровно одной ветвью: сверка чекаута, никогда не снимавшегося, отвечает rc 0
# «основной чекаут чист» вместо именованного отказа. Это инкарнация
# церемониальной дыры «забыл снять снимок» (frontier D(в), класс Н-84
# «неполная раздача»): страж пусто-зелёный ровно там, где церемония ошибается
# руками. Дефект наблюдаем на воротах 5 red_detektor_utechek.sh — «снимок
# отсутствует»: rc 0 вместо rc 1 «снимок отсутствует» (fail-closed нарушен).
