#!/usr/bin/env bash
# НЕ БАРЬЕР: слабая форма детектора контракта 024 — «запись-при-сверке».
# Отличается от честной формы (stab_detektor_chestnyj.sh, правка-круг 2)
# ровно одной ветвью: --check ПОСЛЕ вычисления дельты (вердикт не искажён)
# пишет маркерный файл ВНУТРЬ стерегомого чекаута
# ($CANON/.leak_metka_sverki) — «сверка на месте не пишет» нарушена в
# хвосте. На воротах 1–13 red_detektor_utechek.sh вывод/вердикты честны
# (маркер дописывается после расчёта, каждый ворот — свежий toy-корень);
# дефект наблюдаем на воротах 14 «снятие/сверка не загрязняют стерегомое»:
# байтовое сравнение porcelain до/после видит появившийся «??
# .leak_metka_sverki» (класс Н-48(2): барьер пишет в то, что стережёт).
...
do_check() {
  ...
  # ДЕФЕКТ: пишем маркер в стерегомое ПОСЛЕ расчёта дельты.
  printf 'metka\n' > "$CANON/.leak_metka_sverki"
  if [ -n "$delta" ]; then
    names=""
