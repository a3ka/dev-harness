#!/usr/bin/env bash
# НЕ БАРЬЕР: слабая форма детектора контракта 024 — «без-абс-корня» (обход
# блокера 5 вердикта 4d1d265). Отличается от честной формы
# (stab_detektor_chestnyj.sh, правка-круг 2) ровно одной ветвью: нет отказа
# на относительный аргумент — `cd "$ROOT_ARG" && pwd -P` молча принимает
# относительный путь и резолвит его от случайного cwd (сам Н-85-класс:
# инструмент стоит не там, где заявлен). Манифест честный. Дефект наблюдаем
# на воротах 12 red_detektor_utechek.sh — «относительный корень»: --snapshot
# rel/path → rc 0 вместо rc 1 «корень обязан быть абсолютным» (второй режим
# проверяется той же ветвью). Привязка — кодом этой шапки и кодом пробы (Н-39).
set -uo pipefail
export LC_ALL=C
...
# ДЕФЕКТ: ветки абсолютности нет — относительный путь молча принимается.
