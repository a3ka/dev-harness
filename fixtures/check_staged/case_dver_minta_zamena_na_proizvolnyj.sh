...
#
# Контракт 049 (корень Н-153), клетка «замена на произвольный sha»: подготовка та же,
# что у case_dver_minta_zamena_na_frozen (живой frozen/contracts/040/1 есть), но
# строка 040 заменяется на СЛУЧАЙНЫЙ 40-hex — не tag-object никакого живого тега.
# Форма дельты та же (два хунка, add=1 del=1 на одном NNN) — отличается ТОЛЬКО значение
# sha. Стаб «пропускать любую замену 1+1 на том же NNN» здесь зеленеет.
...
# Красный: замена 040 → случайный 40-hex — отказ двери, как до контракта 049.
set -uo pipefail
. "$(dirname "$0")/_repo.sh"
...
# ── красный: замена 040 на случайный 40-hex ─────────────────────────────────────
