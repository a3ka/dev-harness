...
#
# ЭТА фикстура держит D (наследие F-2, 29116ba): единственный красный вызов — репо с
# ЕДИНСТВЕННОЙ staged-записью типа D (удаление пути, лежащего в HEAD вне зоны). Порча
# --diff-filter, роняющая D (измеренная форма AM), видит на этом входе пустой staged →
# rc 0 «нечего судить: staged пуст» → красное не предъявлено → scoped rc 1. Порча,
# СОХРАНЯЮЩАЯ D, этой фикстурой не ловится — её держит сентинель потерянного типа
# (принцип набора: каждая ПРИЧИНА = свой тип; раннер судит первый красный вызов
# фикстуры, поэтому сентинелей ровно по числу наблюдаемых типов).
#
...
# ── красный репо: ЕДИНСТВЕННАЯ staged-запись — удаление вне-зонного пути ────────
...
"$BARRIER" "$RED_D"  || true # ожидание: rc 1 «ОТКАЗ: вне зоны: offzone_D.txt» — сентинель (ПРИЧИНА);
                             # порча, роняющая D (форма AM, F-2): rc 0 «нечего судить: staged пуст»
