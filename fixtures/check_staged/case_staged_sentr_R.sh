# ПРИЧИНА: вне зоны: offzone_R.txt
#
# СЕНТИНЕЛЬ R полного сентинель-набора (018 пост-заморозочно; закрывает границу AMT,
...
#
# ЭТА фикстура держит R: единственный красный вызов — репо с ЕДИНСТВЕННОЙ staged-записью
# типа R (переименование из зоны в корень; name-only несёт только НОВЫЙ путь, R100 виден
# без -M — замер в шапке case_staged_tipy_vne_zony.sh). Порча --diff-filter, роняющая R,
# видит на этом входе пустой staged → rc 0 «нечего судить: staged пуст» → красное не
# предъявлено → scoped rc 1. Порча, СОХРАНЯЮЩАЯ R, этой фикстурой не ловится — её держит
# сентинель потерянного типа (принцип набора: каждая ПРИЧИНА = свой тип; раннер судит
# первый красный вызов фикстуры, поэтому сентинелей ровно по числу наблюдаемых типов).
#
...
# ── красный репо: ЕДИНСТВЕННАЯ staged-запись — переименование из зоны в корень ───
...
"$BARRIER" "$RED_R"  || true # ожидание: rc 1 «ОТКАЗ: вне зоны: offzone_R.txt» — сентинель (ПРИЧИНА);
                             # порча, роняющая R: rc 0 «нечего судить: staged пуст»
