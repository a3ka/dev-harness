...
#
# СВЕРКА СО СПЕКОЙ (корректив владельца 2026-09-09: конверсия к СПЕКЕ
# contracts/023, не к выводу). Код двери — ПО СПЕКЕ (iii): у1 грамматика
# check_staged.sh:293; у2 тег локально :297, отказ «номер … не выдан — спавн
# мимо реестра» :298; у3а ls-remote точного имени тега :307; у3б манифест по
# ЖИВОЙ шапке origin/main :319 (ls-remote шапки) + :333 (cat-file ПО ШАПКЕ,
# не по локальной ветке/рабочей копии); у3в sha-сверка :341-343; у4 ветка
# wip/<NNN>/architect :356-359; у5 ls-tree refs/heads/main :364; порядок
# у1→у2→у3→у4→у5 (:293→:297→:307→:356→:364). И-4: next_id_peek в scripts/
# пуст (git grep rc 1, прогнано). Ворота спеки — red_dver_po_tegu.sh в0-в10,
# rc 0 на impl (×2 прогона). Прежняя редакция ждала peek-мир, умерший по И-4,
# — сталость фикстуры, конверсия законна; имена отказов и строка двери в
# ассертах — Н-39 по коду; спека имён НЕ ослаблена.
set -uo pipefail
