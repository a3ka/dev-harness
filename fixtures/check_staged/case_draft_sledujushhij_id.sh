...
#     осознанным остатком, закрыты здесь ЦЕЛИКОМ):
...
#       - ПЕРВАЯ цепочка цифр имени ссылки: refs/remotes/origin/wip/<занятый>/x/<вторая>
#         (две цепочки; вторая — VTORAJA_NOMER, производная 10#-арифметикой от
#         параметра); честный берёт первую → staged <max+1> по draft-ветви; стаб
#         последней цепочки называет <вторая>+1 → строки нет → ассерт;
...
#
# ── РЕШЁТКА ЗАКРЫТА ЦЕЛИКОМ (правка 4, аудит по коду next_id_max_for_class) ────
# Оси решётки (арбитраж ebc57db: структурная слабость без ключования на литералы
# фикстуры — законная добыча судьи, закрывается входами):
#   перечисления: источник 1 — TEGSRC; источник 2, локальная половина — VETSRC;
#     удалённая половина — REMSRC (правка 3); источник 3 — BUSY; источник 4 —
#     ISTSRC; половины изолированы ОТДЕЛЬНЫМИ входами;
...
#   грамматика разбора: якорь тега — TEGSUFF; первая цепочка цифр ссылки —
#     PERVAJA; базовое имя rc 1 (цифра за дефисом) — DATG; базовое имя rc 2
#     (не три цифры, номер всё равно считается) — CHETYRE (правка 4).
# Арифметика max+1 закрыта BUSY (значение) и ADJ (соседний max+2); константа и
# off-by-one ловятся теми же входами; октальная канарейка — дефолт 019 (М-1,
# замер 3 арбитража). Различимых осей по коду next_id_max_for_class/next_id_peek
# сверх перечисленных нет: перебран каждый источник и каждое грамматическое
# решение разбора (якорь/цепочка/ветки rc 0-1-2 базового имени); рекурсия
# ls-tree источника 3 оси НЕ образует — всякий файл HEAD попадал в достижимую
# историю добавлением, и источник 4 (полные пути) видит его и без рекурсии.
#
...
# литералы производных запрещены. Вторая цепочка цифр GRM2 (VTORAJA_NOMER) —
...
# Вторая цепочка цифр входа PERVAJA (правка 4): производная <занятый>-1. Пин
# первой цепочки жив, пока следующие номера честного (<занятый>+1) и стаба
# (max(<вторая>, 001)+1) РАЗЛИЧИМЫ — по коду разбора они сходятся ТОЛЬКО при
# равенстве цепочек, поэтому вторая обязана быть МЕНЬШЕ занятого (ассерт ниже;
...
VTORAJA_NOMER="$(printf '%03d' "$((10#$ZANJATYJ_NOMER - 1))")"  # вторая цепочка GRM2
...
# ── пин грамматики 2/4, ПЕРВАЯ цепочка цифр ссылки (правка 4): единственная ────
# запись с числом — удалённая ссылка refs/remotes/origin/wip/<занятый>/x/<вторая>
# с ДВУМЯ цепочками цифр. Честный разбор (([0-9]+), левое совпадение) берёт
# ПЕРВУЮ → staged <max+1> пропущен ПО draft-ветви; стаб круга 5 (последняя
# цепочка) называет <вторая>+1 → строки нет → ассерт (А-73). Хвост ссылки — НЕ
# architect: страж 018 читает только refs/heads/wip/, удалённая ссылка его не
# будит (как у REMSRC).
