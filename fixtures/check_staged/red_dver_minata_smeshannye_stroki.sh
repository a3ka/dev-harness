#!/usr/bin/env bash
# КРАСНОЕ по адверсарию 031-к1 (verdicts/adversary/contracts-031-v1.md, ремонт п.3;
# инвариант — замороженный 031/4, механизм 1, условие 2): СМЕШАННЫЕ строки манифеста.
#
...
#   м1  ВОРОТА 1 (красное СЕГОДНЯ): staged-дельта с ДВУМЯ добавками — валидная
...
#       манифеста» (check_staged.sh:447). Сегодня sed-фильтр (:445) выбрасывает
#       невалидную строку, дверь зелёная «+1 строк» — предъявляемое красное;
...
#   м3  ВОРОТА 2 (красное СЕГОДНЯ): смешанная дельта (валидная + невалидная
...
#       (check_zones.sh:414). Сегодня признание судит подмножество и молчит —
...
# механизм дыры Б1 (вердикт арбитра 9f86d1c). Ворота м2/м3 ассертят %an ==
...
# сегодня красна именованными воротами м1/м3, после all-lines predicate в ОБОИХ
...
# 020–099 — внутри пространства семьи 031 (020–119) и ВНЕ занятых фикстурами 023
...
#               (до фикса красны м1/м3, позитив м0/м2 зелёный).
set -uo pipefail
...
# м2 080-089 · м3 090-094 (валидная) + 095-099 (ASCII) · хвост — имена каталогов.
...
M0="${RD[0]}"; M1V="${RD[1]}"; M1I="${RD[2]}"; M2="${RD[3]}"; M3V="${RD[4]}"; M3I="${RD[5]}"

# Субъект — копия scripts/ под случайным именем (BARRIER_ROOT-паттерн).
BARRIER="$WORK/subj-${RD[6]}"
...
# ПОСЛЕ фикса: rc 1 именем грамматики (all-lines predicate). СЕГОДНЯ: sed-фильтр
# молча выбрасывает невалидную строку, дверь зелёная «+1 строк» — красное.
T="$WORK/kor-${RD[8]}"
make_repo_orchzone "$T"
toy_origin "$T" >/dev/null
...
# ПОСЛЕ фикса: rc 1 «коммит вне зоны … строка не по грамматике манифеста»
# (признание НЕ срабатывает). СЕГОДНЯ: признание судит подмножество и молчит —
# красное.
T="$WORK/korcz-${RD[10]}"
make_repo_orchzone "$T"
toy_origin "$T" >/dev/null
...
printf 'ok: дверь минта 031 mixed-rows — все ворота пройдены (м0..м3)\n'
exit 0
