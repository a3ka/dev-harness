...
#
# Контракт 049 (корень Н-153), клетка «замена на frozen»: дверь минта 031 пропускает
# ЗАМЕНУ строки NNN манифеста, если новый sha — tag-object живого аннотированного
# frozen/contracts/NNN/<v> (локально и на origin), автор orchestrator, ветка main.
# Каркас повторяет живого производителя замены — freeze_contract.sh шаги 8-9:
# строка вычёркивается и дописывается В КОНЕЦ, строка 041 стоит после 040, поэтому
# дельта — два хунка, add=1 del=1 на одном NNN.
# Зелёный (после фикса): 040 → tag-object frozen/contracts/040/1.
# Красный: 040 → tag-object ЧУЖОГО живого frozen-тега frozen/contracts/001/1 (жив,
# аннотирован, на origin) — признаётся только frozen-тег ТОГО ЖЕ номера. Стаб «sha
# любого живого frozen-тега» здесь зеленеет и различим именно на этом входе.
