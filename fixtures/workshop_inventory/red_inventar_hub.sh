...
#             входа идёт ДВУМЯ контрастными проходами — ДВА непредсказуемых
#             НЕПУСТЫХ значения (secrets, на запуск пробы; пустой ZAI — легальный
#             die workshop:177 ДО exec, измерено, не контраст); метринг-переменные
#             также непусты и различны в проходах; hub обязан в операнде ОБОИХ
#             проходов — красен ЛЮБОЙ;
...
# ── контраст окружения живого суда (A-030-1, вердикт f493247) ─────────────────
# Тестовый секрет НЕ константа: оба прохода — непредсказуемые (secrets) НЕПУСТЫЕ
# значения, порождённые на запуск пробы. Пустой ZAI_API_KEY как контраст ИЗМЕРЕН
# невозможным на честном дереве: workshop легально умирает ДО exec (гейт
# авторизации, workshop:177 «ZAI_API_KEY пуст — die»), это не предмет инв. 1;
# метринг-переменные также непусты и различны в обоих проходах. Угадываемый
# sentinel (x, production, любой литерал) не совпадает НИ С ОДНИМ проходом —
# контрrealизация «hub только при ZAI_API_KEY=x» краснеет именованно. Проба не
# экспортирует значения — rt_tree передаёт дереву только явный набор.
ZAI_A="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
ZAI_B="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
MINIMAX_A="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
MINIMAX_B="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
TOKEN_A="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
TOKEN_B="$(python3 -c 'import secrets; print(secrets.token_hex(8))')"
...
# ── живой запуск полного дерева: <дерево> <f0|f1|proj> <файл-захвата> <A|B> ───
# (захват stdout/stderr дерева — гигиена вывода; судом НЕ читается, Т-1).
# Проход окружения A|B — контраст секрета (A-030-1): значения из набора выше,
# дереву передаются только явным перечислением.
rt_tree() {
...
sudit_tree() {  # <дерево> <имя> <A|B>: 0 зелёный | 1 именованный отказ | 2 не дошёл
...
# ── суд главного входа с КОНТРАСТОМ окружения (A-030-1): два прохода ──────────
# СЛОМ доминирует; красен ЛЮБОЙ проход — hub не обусловлен значением секрета.
# Имена проходов БЕЗ слэша: имя входит в путь файла-захвата.
...
# включая легальный v4-сорс, судится исполнением полной командой; A-030-1 —
# ДВА контрастных прохода окружения: непредсказуемый секрет И пустой)
live_rc=0
