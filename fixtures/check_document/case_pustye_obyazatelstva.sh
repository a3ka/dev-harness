...
# Барьер ловит предмет, который ПРИНИМАЕТ spec с required.sections=[] для product profile
# — реальный checker краснит rc=1 с substring «required.sections пуст» (validateSpecSchema).
# Зелёный контроль — реальный предмет против конформного spec (--preflight rc=0).
# Красное — тот же checker против spec с пустым required.sections.
set -euo pipefail
