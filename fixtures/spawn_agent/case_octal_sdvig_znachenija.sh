...
# contracts/023, не к выводу). Код гейта — ПО СПЕКЕ (ii): место после разбора
# аргументов до локальной нумерации (гейт spawn_agent.sh:131-217, автонумерация
# :211 не тронута); локальный тег :134, отказ «номер … не выдан — спавн мимо
# реестра» :135; 3а ls-remote точного имени тега :148; 3б манифест по ЖИВОЙ
# шапке origin/main :154 (ls-remote шапки) + :161 (cat-file ПО ШАПКЕ); 3в
# sha-сверка :166-169; имена отказов «тег … не выдан авторитетом» :184-194 и
# «авторитет недоступен» :180 (fail-closed). Ворота спеки —
# red_gejt_javnogo_nomera.sh в1-в9, rc 0 на impl (×2 прогона). Прежний зелёный
# контроль (голый --nnn 19 без резерва) ждал до-023 мир — сталость фикстуры,
# конверсия законна; предмет октал-пина (10#-паддинг :132) спекой сохранён.
set -uo pipefail
