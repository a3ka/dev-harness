...
#
# После реализации и конверсии red_regress → case_regress (протокол раннера,
# А-82) проба ВЕРНУЛАСЬ к scoped-прогонам в тех же подставных корнях: фаза 4
# судит мутант «теряет merge-принесённые» РАННЕРОМ — на toy-копии дерева
# scoped-ключом case_regress; красное предъявление исчезает у мутанта целиком
# («барьер остался зелёным на обманном дереве»). red_mera остаётся прямым
# файлом навсегда (мера — не барьер, V2): его фазы гоняют red-файл ПРЯМЫМ
# запуском в подставных корнях.
#
...
#             исключена) → scoped-прогон case_regress раннером в том же
#             подставном корне → rc 1: toy с единственным merge-принесённым
#             нарушением остаётся зелёным, красное не предъявлено;
...
#             там, Н-39).
#   фаза 6 — слабая «зашитая пара» (обход из вердикта адверсария 021 v2,
#             44e81e7: константа пары вместо вычисления): честные моментные
#             проверки для всех пар, КРОМЕ 005/007 — для неё exit 0 без
#             вычисления → red_mera rc 1: умирает на воротах
#             «последовательная пара других номеров 005/007» (дефект
#             наблюдаем там, Н-39).
# Слабая «хвост до HEAD» отдельной фазы не имеет: это ДО-021 поведение
# честного дерева; её ловит вход 1 case_regress (ворота до первой пары).
#
...
mk_toy() {  # <каталог> <файл-меры>
  ...
  # раннер и селектор — для scoped-прогона фазы 4 (семья судится РАННЕРОМ)
  cp "$ROOT/scripts/verify_antiplacebo.sh" "$ROOT/scripts/scope_select.sh" "$1/scripts/"
...
# Scoped-прогон РАННЕРОМ в подставном корне (А-78: свежий toy каждый прогон).
# Скратч раннера — явный, вне toy-корня; раннер оставляет RUN при отказе
# (А-19: die-причина диагностируема) — прогон красный ПО КОНСТРУКЦИИ, скратч
# фазы убирает проба.
...
out="$(VERIFY_ANTIPLACEBO_SCRATCH="$T/r4-scratch" \
  bash "$T/r4/scripts/verify_antiplacebo.sh" "$T/r4" \
       --scope check_zones/case_regress_posledovatel_naja_istorija 2>&1)" && rc=0 || rc=$?
rm -rf "$T/r4-scratch"
[ "$rc" -ne 0 ] || fail "слабая «теряет merge-принесённые» не поймана: scoped case_regress rc=0
$out"
...
# ── фаза 6: слабая «зашитая пара» (обход 44e81e7: константа пары) ─────────────
mk_toy "$T/r6" "$HERE/stab_mera_zashitaja_para.sh"
out="$(run_red "$T/r6" red_mera_parallelnosti_okon.sh)" && rc=0 || rc=$?
[ "$rc" -ne 0 ] || fail "слабая «зашитая пара» не поймана: red_mera rc=0
$out"

printf 'пойманы: всегда-0, пусто-зелёная, теряет-merge-принесённые (scoped-прогоном раннера), лексикографическая, зашитая-пара; честная форма меры (моменты, %%ct) проходит\n' >&2
exit 0
