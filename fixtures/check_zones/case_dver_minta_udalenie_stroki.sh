...
#
# Контракт 049 (корень Н-153), клетка «удаление строки», близнец в check_zones.sh:
# подготовка та же, что у case_dver_minta_zamena_na_frozen (живой
# frozen/contracts/040/1 есть), но коммит orchestrator ВЫЧЁРКИВАЕТ строку 040 без
# замены: add=0 del=1. Стаб «del_count ≤ 1 без требования ровно одной +строки того же
# NNN» здесь зеленеет.
# Зелёный контроль: история из честных минтов и заморозки — барьер зелёный ДО порчи.
# Красный: тот же toy + коммит удаления строки 040 — «коммит вне зоны» с причиной
# двери, как до контракта 049.
set -uo pipefail
. "$(dirname "$0")/_repo.sh"
...
# ── красный: коммит удаления строки 040 ─────────────────────────────────────────
perepis "$R" 040 ""
"$BARRIER" "$R" || true
