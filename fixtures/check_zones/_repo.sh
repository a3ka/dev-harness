...
# Заморозка версии N подставного контракта ПО ПРОЦЕДУРЕ: правка объявления + вердикт
# критика accept + тег. СПАСЕНО-строки попадают в заморозку тем же путём, что и ЗОНА:
# барьер читает их из блоба ВЫСШЕЙ заморозки — потому вторая стадия фикстуры требует
# СЛЕДУЮЩУЮ версию, а не повторный тег.
freeze_ver() {  # <корень> <версия> <новое объявление целиком>
  ...
  commit_all "$r" "контракт v$v"
  g "$r" tag -a "frozen/contracts/001/$v" -m "контракт v$v утверждён"
}

freeze_v2() { freeze_ver "$1" 2 "$2"; }

# Второй контракт подставного репо для оконных входов 021 (И-1, И-5, case_regress):
# файл contracts/<NNN>-y.md + коммит основания + тег frozen/contracts/<NNN>/1.
# Коммитит Фикстура: автор основания не объявлен ни в одном окне и судом не
# разбирается — окно открывает ТОЛЬКО тег заморозки, как и у make_repo.
add_contract() {  # <корень> <NNN> <строка объявления целиком>
  local r="$1" n="$2" declaration="$3"
  printf '# контракт %s\n\n## Предмет\nподставной предмет\n\n## Критерий готовности\nкоманда с кодом возврата\n\n## Исполнители и зоны\n%s\n' \
    "$n" "$declaration" > "$r/contracts/$n-y.md"
  commit_all "$r" "основание: контракт $n"
