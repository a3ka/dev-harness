...
#
# Форма веток (Н-107, ИЗ-1 доклада Implementer030): слитые строятся mk_merged'ом
# С РАБОТОЙ — собственный коммит на ветке, ≥2 события reflog, снос БЕЗУСЛОВЕН.
# Ожидания не менялись: снос по-прежнему приписан СЛИТОСТИ, а не истечению грации
# пустых (слитая-пустая пережила бы GC в грации и солгала бы в обе стороны).
# Зависшая wip/010 имеет собственный коммит (2 события reflog) и уцелела как
# ЗАВИСШАЯ — unreachable из HEAD, не как «пустая в грации».
#
