...
#
# `make_repo <корень>` собирает минимальное дерево с HEAD=main и одним коммитом-основанием;
# зон и заморозок GC не читает (он ходит только по refs/heads/wip/), но контракт и вердикт
...
# mk_merged — ветка wip/*, СЛИТАЯ С РАБОТОЙ: собственный коммит на ветке, затем main
# поглощает её merge-коммитом (прецедент red_gonka_svezhij_spavn_vyzhivaet.sh:94-99).
# Tip ДОСТИЖИМ из HEAD — GC обязан снести; reflog ветки ≥2 события (создание + коммит):
# по Н-107 это «слитая с работой» — сносится БЕЗУСЛОВНО, грация пустых её не держит.
# Прежняя форма `branch main` (ровно 1 событие reflog) после Н-107 — «пустая»: моложе
# грации, переживает GC, и снос не наблюдается (ИЗ-1 доклада Implementer030). Снос
# обязан быть приписан СЛИТОСТИ, а не истечению грации — форма ниже это даёт при
# любом --wip-grace-hours.
mk_merged() {  # <корень> <ветка>
  local r="$1" br="$2"
  g "$r" branch "$br" main
  g "$r" checkout -q "$br"
  g "$r" commit -q --allow-empty -m "предмет слитой $br"
  g "$r" checkout -q main
  g "$r" merge -q --no-ff -m "merge $br" "$br"
}
