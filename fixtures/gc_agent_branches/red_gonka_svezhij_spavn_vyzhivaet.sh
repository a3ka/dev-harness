...
# Круг 2 (вердикт f493247, A-030-3): ДОПОЛНИТЕЛЬНО второй свежий пустой достижимый
# спавн wip/999/fresh-<urandom> с СВОИМ worktree и независимым mtime (−60с), и
# wip/999/nolog-<urandom> без reflog-файла: имена НЕПРЕДСКАЗУЕМЫ и отсутствуют в
# тексте пробы — жнец обязан вычислять инвариант (пустота = 1 reflog-событие,
# возраст = mtime reflog), а не узнавать константы игрушки; default/N/fail-closed
# повторяются на непредсказуемых именах (ассерты 2b, 4b, 8b, 11b).
#
...
#   2b) ВТОРОЙ свежий пустой спавн с НЕПРЕДСКАЗУЕМЫМ именем ЖИВ (A-030-3);
...
#   4b) no-log с НЕПРЕДСКАЗУЕМЫМ именем ЖИВА — fail-closed имененезависим;
...
#   8b) связки на НЕПРЕДСКАЗУЕМЫХ именах: fresh2 + «граци», nolog2 + «reflog»;
#   B (--wip-grace-hours 3):
#   9)  gc rc 0;
...
#   11b) непредсказуемые: fresh2 ЖИВА (~1мин < 3ч), её worktree цел, nolog2 ЖИВА;
...
#   S-ignore-N    — «N принимается, но игнорируется» (обход круга 1): умирает
#                   на ассерте 10 (прогон B);
#   S-zero-grace  — «грация ноль/минуты: всё пустое сносится сразу»: умирает
#                   на ассерте 3 (young23 моложе 24ч);
...
#   S-imena       — «жнец узнаёт КОНСТАНТНЫЕ имена игрушки, а не вычисляет
#                   инвариант» (обход A-030-3, вердикт f493247: контржнец сохранял
#                   только wip/031/architect, wip/030/young23, wip/030/no-log — и
#                   прошёл приёмку круга 1): умирает на ассерте 2b — свежий пустой
#                   спавн с urandom-именем сносится; 4b/8b/11b — fail-closed,
#                   stderr-связка и N-грация на том же непредсказуемом имени;
...
# ── A-030-3 (вердикт f493247): непредсказуемые имена — urandom-суффикс ─────────
# Имени нет в тексте пробы: поимённый жнец не может его сохранить. Отказ источника
# энтропии — громкий rc 2 (деградация до предсказуемого имени запрещена).
SUF="$(head -c 8 /dev/urandom 2>/dev/null | od -An -tx1 | tr -d ' \n')"
[ -n "$SUF" ] || { printf 'NOT_IMPLEMENTED: непредсказуемый суффикс имени не получен\n' >&2; exit 2; }
FRESH2="wip/999/fresh-$SUF"
zgi "$R" branch "$FRESH2" main
zgi "$R" worktree add "$WORK/wt-fresh2" "$FRESH2" >/dev/null 2>&1
touch -d "@$(( $(date +%s) - 60 ))" "$R/.git/logs/refs/heads/$FRESH2"   # независимый mtime

# fail-closed на непредсказуемом имени: reflog-файл удалён → выживает.
NOL2="wip/999/nolog-$SUF"
zgi "$R" branch "$NOL2" main
rm "$R/.git/logs/refs/heads/$NOL2"
...
if ! ref_zhiv wip/031/architect; then
  printf 'ОТКАЗ: свежая пустая wip/031/architect снесена — гонка Н-107: жнец сносит свежеспавненную ветку\n%s\n%s\n' "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1
fi
[ -d "$WORK/wt-spawn" ] || { printf 'ОТКАЗ: worktree свежего спавна снесен вместе с веткой (Н-107)\n%s\n%s\n' "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1; }

# 2b) ВТОРОЙ свежий пустой спавн с НЕПРЕДСКАЗУЕМЫМ именем ЖИВ (A-030-3: S-imena —
#     поимённый жнец сносит её: имени нет в тексте пробы, сохранить по имени нельзя).
if ! ref_zhiv "$FRESH2"; then
  printf 'ОТКАЗ: свежая пустая ветка с непредсказуемым именем %s снесена — жнец узнаёт КОНСТАНТЫ игрушки, а не вычисляет инвариант (A-030-3, f493247)\n%s\n%s\n' "$FRESH2" "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1
fi
[ -d "$WORK/wt-fresh2" ] || { printf 'ОТКАЗ: worktree непредсказуемого спавна %s снесен вместе с веткой (A-030-3)\n%s\n%s\n' "$FRESH2" "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1; }
...
# 4b) no-log с НЕПРЕДСКАЗУЕМЫМ именем ЖИВА — fail-closed не узнаёт имена (A-030-3).
ref_zhiv "$NOL2" || { printf 'ОТКАЗ: ветка без reflog с непредсказуемым именем %s снесена — fail-closed обязан быть имененезависимым (A-030-3)\n%s\n%s\n' "$NOL2" "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1; }

# 5) aged25 (−25ч) СНЕСЕНА при дефолте: дефолт 24ч, а не фиксированные 48 (S-48h).
if ref_zhiv wip/030/aged25; then
  printf 'ОТКАЗ: пустая wip/030/aged25 (−25ч) пережила gc — дефолтная грация ≥ 48ч, а не 24ч\n%s\n%s\n' "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1
fi

# 6) Старая пустая снесена — гигиена упавших спавнов (S-nikogda умирает здесь).
if ref_zhiv wip/030/oldcrash; then
  printf 'ОТКАЗ: старая пустая wip/030/oldcrash (−3 дня) пережила gc — гигиена упавших спавнов умерла\n%s\n%s\n' "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1
fi

# 7) Молодая слитая снесена — чистка close-out не заторможена (S-vsyo-gracia).
if ref_zhiv wip/029/merged; then
  printf 'ОТКАЗ: молодая слитая wip/029/merged пережила gc — грация тормозит чистку close-out (инвариант 3.5)\n%s\n%s\n' "$(cat "$outA")" "$(cat "$errA")" >&2; exit 1
fi
...
# 8b) Связки на НЕПРЕДСКАЗУЕМЫХ именах (A-030-3): stderr обязан объяснять выживание
#     ЛЮБОЙ ветки, не только известных игрушке.
svjazka "$errA" "$FRESH2" 'граци' || { printf 'ОТКАЗ: stderr не объясняет выживание непредсказуемого спавна %s — нет строки-связки с «граци» (A-030-3)\nstderr:\n%s\n' "$FRESH2" "$(cat "$errA")" >&2; exit 1; }
svjazka "$errA" "$NOL2" 'reflog' || { printf 'ОТКАЗ: stderr не называет причину выживания %s — нет строки-связки с «reflog» (A-030-3)\nstderr:\n%s\n' "$NOL2" "$(cat "$errA")" >&2; exit 1; }
...
if ref_zhiv wip/030/young23; then
  printf 'ОТКАЗ: пустая wip/030/young23 (−23ч ≥ 3ч) пережила gc при N=3 — флаг принимается, но не применяется\n%s\n%s\n' "$(cat "$outB")" "$(cat "$errB")" >&2; exit 1
fi
...
# 11b) Непредсказуемые имена при N: свежая (~1мин < 3ч) ЖИВА с worktree, no-log ЖИВА
#      (A-030-3: N-грация и fail-closed применяются к ЛЮБОМУ имени).
ref_zhiv "$FRESH2" || { printf 'ОТКАЗ: свежая пустая %s снесена при N=3 (возраст ~1мин < 3ч) — грация N не применяется к непредсказуемому имени (A-030-3)\n%s\n%s\n' "$FRESH2" "$(cat "$outB")" "$(cat "$errB")" >&2; exit 1; }
[ -d "$WORK/wt-fresh2" ] || { printf 'ОТКАЗ: worktree непредсказуемого спавна снесен при N=3 (A-030-3)\n%s\n%s\n' "$(cat "$outB")" "$(cat "$errB")" >&2; exit 1; }
ref_zhiv "$NOL2" || { printf 'ОТКАЗ: ветка без reflog %s снесена при N=3 — fail-closed не зависит от имени и грации (A-030-3)\n%s\n%s\n' "$NOL2" "$(cat "$outB")" "$(cat "$errB")" >&2; exit 1; }
