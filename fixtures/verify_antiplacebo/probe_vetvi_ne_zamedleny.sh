...
#            барьера (число из вывода, не из этого файла), ФАКТОР=6 — запас на
#            контеншен и разброс отношения полный/база (замер пачки: честное
#            дерево 38–55 при разрешённых 90; контрпример «sleep 2 в proxy_up +
#            sleep 20 в ветви» — 172); замедления ниже порога терпимы текстом
#            контракта (арбитраж §Границы 4 — цена против флейка меры). Обе
#            меры сняты подряд в одном прогоне и масштабируются вместе. Замер
#            арбитража: независимая база ~318 мс; честное дерево и контрпример
#            расходятся по порогу кратно — знаменатель больше не растёт вместе
#            с числителем.
...
if [ -z "$b_ms" ]; then
  ...
  if [ "$full_rc" != 0 ]; then
    ...
    printf 'фактор %d · ветвей %d (из вывода) · независимая база мс %d (медиана %d/%d/%d) · полный мс %d · порог мс %d\n' \
  ...
  if [ "$full_rc" != 0 ]; then
    ...
    printf 'фактор %d · ветвей %d (из вывода) · независимая база мс %d (медиана %d/%d/%d) · полный мс %d · порог мс %d\n' \
      "$FAKTOR" "$full_N" "$b_ms" "$b1" "$b2" "$b3" "$full_ms" "$porog" >&2
    if [ "$full_ms" -gt "$porog" ]; then
      bad "полный барьер ${full_ms} мс > порога ${porog} мс (фактор ${FAKTOR} × ${full_N} ветвей × независимая база ${b_ms} мс) — ветви замедлены кратно базе среды, И-4 нарушен"
    else
      ok "полный барьер ${full_ms} мс внутри порога ${porog} мс (фактор ${FAKTOR} × ${full_N} ветвей × независимая база ${b_ms} мс) — замедления нет"
    fi
  fi
fi
