...
# Дерево пересобирается на одном пути: PATH и стаб-omp остаются согласованы.
# Зелёный: фикстуры закоммичены РАНЬШЕ скилов, все авторы — architect → 0.
# Красное 1: скилы без фикстур на родителе → код 1.
# Красное 2 (вердикт milestone-003-zakrytie §3): implementer коммитит правку фикстуры
# ПОСЛЕ стартового коммита архитектора — судятся все коммиты, не только первый.
set -euo pipefail
. "$(dirname "$0")/_repo.sh"

mk() { # <root> <fixtures_first>
  local r="$1" first="$2" f s
  mkdir -p "$r/skills/grilling" "$r/skills/writing-for-agents" "$r/skills/tdd" "$r/skills/diagnosing-bugs" "$r/fixtures/check_skills" "$r/tmp" "$r/.agents/skills"
...
rm -rf "$R"
mk "$R" yes
printf '# probe\n' >> "$R/fixtures/check_skills/case_b_imja_kataloga.sh"
GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_SYSTEM=/dev/null \
  git -C "$R" -c user.name=implementer -c user.email=implementer@dev-harness.local add -A fixtures/
GIT_CONFIG_GLOBAL=/dev/null GIT_CONFIG_SYSTEM=/dev/null \
  git -C "$R" -c user.name=implementer -c user.email=implementer@dev-harness.local commit -q -m 'probe fixture change'
"$BARRIER" --live "$R"
