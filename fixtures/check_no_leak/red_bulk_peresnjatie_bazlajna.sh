...
# предмет предъявляется ПРЯМЫМ запуском, приёмка — ДВА прогона подряд с РАЗНЫМИ
# случайными входами. Перенесён из fixtures/check_judge_gate/ (семья БАРЬЕРА
# check_judge_gate.sh — другой предмет; precision-гейт 043 задача (б); прецедент
# fixtures/self_contained_cwd/, 037); фикстура 031 red_peresnjatie_bazlajna.sh там
# остаётся — граница новизны гейта (блоб тега минта) её не судит. Ядро Н-139
# реализовано (bc4085a), carve-out Н-147 реализован (c7de0ef): все ворота б0–б10 и
