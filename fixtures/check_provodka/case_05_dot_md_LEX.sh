#!/usr/bin/env bash
# ПРИЧИНА: проводка: role-канал обязан ссылаться строго на roles/<роль>.md, получено: roles/.md
# ОКРУЖЕНИЕ: BARRIER_ROOT=$WORK
# case_05 — RED: канал role=roles/.md (круг 3б, пустая база). Л ловит:
# `role_component_ok` возвращает 1 (база после `.md` пуста).
set -uo pipefail
...
if [ -z "${BARRIER:-}" ]; then
  REPO="$(cd "$HERE/../.." && pwd -P)"
  WORK="$(mktemp -d "${TMPDIR:-/tmp}/case05_038.XXXXXX")"
...
- charter=AGENTS.md §Воркфлоу майлстоуна «Norma stroki ustava v igrushke R.»'
commit_all "$R" 'r5: pustaja baza'
RED_OUT="$("$BARRIER" "$R" "$R/contracts/001-x.md" 2>&1)"; RED_RC=$?

if [ -z "${BARRIER:-}" ] || [ "${BARRIER:-x}" = "$REPO/scripts/check_provodka.sh" ]; then
  [ "$RED_RC" -eq 1 ] || { printf 'FAIL: case_05 red rc=%s, ожидался 1\n%s\n' "$RED_RC" "$RED_OUT" >&2; exit 1; }
  printf '%s' "$RED_OUT" | grep -Fq 'проводка: role-канал обязан ссылаться строго на roles/<роль>.md, получено: roles/.md' \
    || { printf 'FAIL: case_05 red причина не названа\n%s\n' "$RED_OUT" >&2; exit 1; }
  printf 'case_05: прямой rc 1, причина названа дословно\n' >&2
  exit 0
fi
exit 0
