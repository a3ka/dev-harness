...
# В4-3, подмена «литерал — ДАННЫЕ исполненной команды» (арбитраж 036d1f1
# замер 1, вход probe.py data_arg; красный вход 8e6ec26 «литерал-как-данные»):
# проба «true scripts/check_judge_gate.sh; exit 0» проходит гейт по grep -F
# литерала в трассе — xtrace честно печатает «+ true scripts/…», литерал есть
# в ИСПОЛНЕННОЙ строке, но стоит в позиции АРГУМЕНТА, вызова нет.
# Различающий критерий (036d1f1): командная позиция литерала в ИЗОЛИРОВАННОЙ
# трассе (BASH_XTRACEFD) — литерал в позиции аргумента любой команды, кроме
# обёрток белого списка, командной позиции не занимает.
...
#   S-zhivyj-literál (G): живой вызов каноническим литералом «bash <литерал> …»
#     — командная позиция занята; rc 0 ДО и ПОСЛЕ фикса, живой вызов обязан
#     оставаться зелёным;
#   S-literál-dannye (M): «true <литерал>» + exit 0; субъект ДО фикса —
#     git show 8e6ec26:scripts/check_spec_ready.sh (grep литерала по трассе),
#     исторический свидетель — даёт rc 0 «OK» (свидетельство дыры); честный
#     субъект обязан отказать rc 1 именованным «зелёный контроль семьи
#     не предъявлен»;
...
#     на M специфична литералу-данным, а не «пропускает всё».
set -euo pipefail
...
# литерал-данные (замер 1 вердикта 036d1f1, строка data_arg → rc 0 OK).
...
# ── S-zhivyj-literál (G): живой вызов каноническим литералом — зелёный ───────
G="$WORK/s_zhivogo_literala"; make_toy "$G"; sbor_semi "$G"
printf 'printf "ФРАЗА ЖИВОГО ЛИТЕРАЛА\\n" >&2\nexit 1\n' > "$G/p_ok.sh"
cat > "$G/fixtures/check_judge_gate/case_zhivoj_kontrol.sh" <<'PROBE'
#!/usr/bin/env bash
# живой зелёный контроль семьи, каноническая форма В4-2 «литерал»:
# bash <литерал относительного пути> — командная позиция занята вызовом
set -uo pipefail
bash scripts/check_judge_gate.sh . зелёный
PROBE
cat > "$G/contracts/997-v43-literal-dannye-zhivoj.md" <<'MD'
# 997 — В4-3: семья с живым вызовом барьера литералом

## Приёмочный критерий
- `bash p_ok.sh` → красная: ФРАЗА ЖИВОГО ЛИТЕРАЛА
- `bash fixtures/check_judge_gate/case_zhivoj_kontrol.sh`

## Исполнители и зоны
ЗОНА architect: fixtures/check_judge_gate/case_zhivoj_kontrol.sh
MD
commit_all "$G" 'живой вызов литералом'
"$BARRIER" "$G" contracts/997-v43-literal-dannye-zhivoj.md

# ── S-literál-dannye (M): литерал — аргумент no-op, вызова нет ───────────────
M="$WORK/s_literala_dannye"; make_toy "$M"
cp "$REPO/scripts/check_judge_gate.sh" "$M/scripts/check_judge_gate.sh"
printf 'printf "ФРАЗА ЛИТЕРАЛА-ДАННЫХ\\n" >&2\nexit 1\n' > "$M/p_ok.sh"
cat > "$M/fixtures/check_judge_gate/case_literal_dannye.sh" <<'PROBE'
#!/usr/bin/env bash
# «зелёный контроль» семьи judge_gate: литерал исполнен как ДАННЫЕ, вызова нет
# (вход probe.py data_arg арбитража 036d1f1 — дословно)
true scripts/check_judge_gate.sh
exit 0
PROBE
cat > "$M/contracts/998-v43-literal-dannye.md" <<'MD'
# 998 — В4-3: литерал барьера как данные команды вместо вызова

## Приёмочный критерий
- `bash p_ok.sh` → красная: ФРАЗА ЛИТЕРАЛА-ДАННЫХ
- `bash fixtures/check_judge_gate/case_literal_dannye.sh`

## Исполнители и зоны
ЗОНА architect: fixtures/check_judge_gate/case_literal_dannye.sh
MD
commit_all "$M" 'литерал-данные: аргумент no-op без вызова'

# Свидетельство дыры: субъект ДО фикса кредитует литерал-данные — гейт зелёный
# на обмане (rc 0, последняя строка OK), вызов барьера не исполнялся ни разу.
run_gate "$STAB" "$M" contracts/998-v43-literal-dannye.md
if [ "$GATE_RC" -ne 0 ]; then
  printf 'ОТКАЗ: стаб 8e6ec26 (grep литерала по трассе) не воспроизвёл пропуск литерала-данных — rc %s, свидетельство требует rc 0:\n%s\n' "$GATE_RC" "$GATE_OUT" >&2
  exit 1
fi
...
printf 'свидетельство: ДО фикса литерал-данные проходят — rc 0, «OK»\n' >&2

# Красное предъявление: честный субъект обязан отказать именованным «зелёный
# контроль семьи не предъявлен» — зачёт несёт КОМАНДНАЯ ПОЗИЦИЯ литерала,
# не его вхождение в аргументы исполненной команды (036d1f1).
run_gate "$BARRIER" "$M" contracts/998-v43-literal-dannye.md
if [ "$GATE_RC" -ne 1 ]; then
  printf 'ОТКАЗ: литерал-данные (true <литерал>; exit 0) засчитаны живым контролем — rc %s, гейт зелёный на обмане:\n%s\n' "$GATE_RC" "$GATE_OUT" >&2
  exit 1
fi
printf '%s\n' "$GATE_OUT" | grep -Fq 'зелёный контроль семьи не предъявлен' || {
  printf 'ОТКАЗ: литерал-данные отказаны rc 1, но без именованной причины «зелёный контроль семьи не предъявлен»:\n%s\n' "$GATE_OUT" >&2
  exit 1
}
printf 'красное предъявление: литерал-данные отказаны именованным\n' >&2
...
# специфична литералу-данным (в трассе есть, в командной позиции нет), а не
# слепоте.
